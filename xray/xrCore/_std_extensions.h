...
#ifdef _EDITOR
IC void strcpy_s(char* strDestination, size_t sizeInBytes, const char* strSource) { strcpy(strDestination, strSource); }
...
IC void strcat_s(char* strDestination, size_t sizeInBytes, const char* strSource) { strcat(strDestination, strSource); }
...
IC LPCTSTR get_token_name(xr_token* tokens, int key)
{
...
IC int get_token_id(xr_token* tokens, LPCTSTR key)
{
    for (int k = 0; tokens[k].name; k++)
        if (wcsicmp(tokens[k].name, key) == 0)
            return tokens[k].id;
    return -1;
}
...
#else
IC int xr_strcmp(const TCHAR* S1, const TCHAR* S2) { return (int)wcscmp(S1, S2); }
#endif

XRCORE_API TCHAR* timestamp(string64& dest);
