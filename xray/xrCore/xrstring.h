...
public:
	...
	// assignment & accessors
                                                shared_str& operator=(LPCTSTR rhs)
                                                {
                                                    _set(rhs);
                                                    return (shared_str&)*this;
                                                }
	shared_str&			operator=	(shared_str const &rhs)			{	_set(rhs);	return (shared_str&)*this;			}
	LPCTSTR				operator*	() const						{	return p_?p_->value:0;							}
	bool				operator!	() const						{	return p_ == 0;									}
	TCHAR				operator[]	(size_t id)						{	return p_->value[id];							}
        LPCTSTR c_str() const { return p_ ? p_->value : 0; }
        LPCTSTR data() const { return p_ ? p_->value : TEXT(""); }
	...
	bool				equal		(const shared_str & rhs) const	{	return (p_ == rhs.p_);							}
        shared_str& __cdecl sprintf(LPCTSTR format, ...)
        {
		string4096 	buf;
		va_list		p;
		va_start	(p,format);
		#ifdef UNICODE
                int vs_sz = _vswprintf(buf, format, p);
                buf[sizeof(buf) - 1] = 0;
		#else
...
IC int	xr_strcmp		(const shared_str & a, const TCHAR* b)				{ return xr_strcmp(*a,b);	}
IC int	xr_strcmp		(const TCHAR* a, const shared_str & b)				{ return xr_strcmp(a,*b);	}
...
IC void	xr_strlwr		(shared_str& src)									{ if (*src){LPTSTR lp=xr_strdup(*src); xr_strlwr(lp); src=lp; xr_free(lp);} }
