...
struct XRCORE_API IIniFileStream
{
	...
	virtual void	__stdcall	w_stringZ		( LPCTSTR S)					= 0;
	...
	virtual void	__stdcall	r_string		(LPTSTR dest, u32 dest_size)	= 0;
...
{\
if(inistream)\
	R_ASSERT3(0,#what_to_do,TEXT("not implemented"));\
}
...
public:
	...
	IC void w_stringZ			( LPCTSTR S )	{ W_guard g(&w_allow); w(S,(u32)xr_strlen(S)+1);	INI_W(w_stringZ(S));		}
	...
	void		r_stringZ		(LPTSTR S );
	void		r_stringZ		(xr_string& dest );
	void 		r_stringZ		(shared_str& dest);
	
	void		skip_stringZ	();
	
	void		r_stringZ_s		(LPTSTR string, u32 size);
