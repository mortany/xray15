...
BOOL CNotifyApp::InitInstance() {
	...
	MaxSDK::Util::GetPrivateProfileString(iniMAILTARGETS, iniFAILURE,_T(""),
		(TCHAR*)&buffer,_countof(buffer),m_sINIname);
	if (buffer[0])
		failureTarget = buffer;
	MaxSDK::Util::GetPrivateProfileString(iniMAILTARGETS, iniPROGRESS,_T(""),
		(TCHAR*)&buffer,_countof(buffer),m_sINIname);
	if (buffer[0])
		progressTarget = buffer;
	MaxSDK::Util::GetPrivateProfileString(iniMAILTARGETS, iniCOMPLETION,_T(""),
		(TCHAR*)&buffer,_countof(buffer),m_sINIname);
	if (buffer[0])
