...
void FooControl::adjust_control(int& current_y)
{
	if (parent_rollout == nullptr || parent_rollout->page == nullptr)
		return;

	WORD spinner_id = control_ID;
	WORD label_id = spinner_id + 1;
	WORD edit_box_id = label_id + 1;
	...
	Value* fw = control_param(fieldwidth);
	int spin_width = (fw == &unsupplied) ? parent_rollout->current_width / 2 : fw->to_int() + 10;
	DLGetTextExtent(parent_rollout->rollout_dc, label_text, &size, true);
	int lbl_width = size.cx;
	int orig_width = lbl_width + spin_width;

	pos.width = orig_width + 2;
	pos.left = pos.left + parent_rollout->current_width - pos.width;
	pos.height = parent_rollout->text_height + 3;
	process_layout_params(parent_rollout, &pos, current_y);

	int cex = (fw == &unsupplied) ? pos.width * lbl_width / orig_width : pos.width - spin_width;
	cex = min(cex, pos.width);
	width = lbl_width; height = parent_rollout->text_height;
	left = pos.left + cex - width - 1; top = pos.top;
	...
	HWND edit_box = GetDlgItem(parent_rollout->page, edit_box_id);
	if (edit_box)
	{
		width = pos.width - cex - 13; height = parent_rollout->text_height + 3;
		left = pos.left + cex + 1;

		InvalidateRect(edit_box, NULL, TRUE);
		SetWindowPos(edit_box, NULL, GetUIScaledValue(left), GetUIScaledValue(top), GetUIScaledValue(width), GetUIScaledValue(height), SWP_NOZORDER);
	}

	HWND spinner = GetDlgItem(parent_rollout->page, spinner_id);
	if (spinner)
	{
		left += width; width = 12;

		InvalidateRect(spinner, NULL, TRUE);
		SetWindowPos(spinner, NULL, GetUIScaledValue(left), GetUIScaledValue(top), GetUIScaledValue(width), GetUIScaledValue(height), SWP_NOZORDER);
	}
}
