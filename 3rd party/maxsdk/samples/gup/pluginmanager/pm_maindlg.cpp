...
void newPluginLoadedProc(void *param, NotifyInfo *info)
{
   if (info->intcode==NOTIFY_PLUGIN_LOADED)
   {
      ...
      DllDesc *dsc = static_cast<DllDesc*>(info->callParam);
...
void PlugMgrUtility::InsertDirectorySelector(HWND pd, int index, const TCHAR* plugInDesc, const TCHAR* plugInPath)
{
...
void PlugMgrUtility::Init(HWND hWnd)
{
	...
	for(;i<nPIE;i++)
	{
		const TCHAR* plugInDescStr = ip->GetPlugInDesc(i);
		//" Standard MAX plug-ins" are defined as INI_KEY_STDPLUGS_DESC in ..\include\ini_section_key_names.h.
		//"Additional MAX plug-ins" (and "Civil View Plugins") are read from file Plugin.UserSettings.ini. 
		...
		else if(_tcscmp(plugInDescStr, _T("Civil View Plugins")) == 0)
			_tcsncpy(pluginDescBuf, GetString(IDS_CIVIL_VIEW_PLUGS_DESC), MAX_PATH);
		else
			_tcsncpy(pluginDescBuf, plugInDescStr, MAX_PATH);

		std::set<TSTR> s;
		s.emplace(MaxSDK::Util::Path(ip->GetPlugInDir(i)).Normalize().ConvertToLowerCase().GetString());
		pdrec.push_back(PluginBucket(true, s));
		InsertDirectorySelector(pd, i, plugInDescStr, ip->GetPlugInDir(i));
	}
	for (size_t j = 0; j < nPNativeBundle; j++)
	{
		const MaxSDK::PluginPackageInfo* pInfo = pManager->GetPackage(j);
		...
		if (pName != nullptr && pPath != nullptr && nPluginComponents > 0)
		{
			...
			pdrec.push_back(PluginBucket(true, s));
			InsertDirectorySelector(pd, i++, pName, pPath);
		}
	}
