...
void EditPolyObject::GetWorldBoundBox(TimeValue t,INode* inode, ViewExp *vpt, Box3& box, ModContext *mc) {
	...
	box.Init();
	if (ShowGizmo()|| GetSelectMode()==PS_SUBOBJ_SEL || GetSelectMode()==PS_MULTI_SEL || (selLevel != EP_SL_OBJECT))
	{
		UpdateEverything (t);
		Matrix3 tm = inode->GetObjectTM(t);
...
{
	int useSubdiv = false;
	if (pblock) pblock->GetValue(ep_surf_subdivide, 0, useSubdiv, FOREVER);
	EditPolyObject* nonConst = const_cast<EditPolyObject*>(this);
	int selMode = nonConst->GetSelectMode();
	if (useSubdiv || nonConst->EpPreviewOn() || 
		selMode == PS_SUBOBJ_SEL || selMode == PS_MULTI_SEL ||
		(subobjectPickMode && subobjectPickMode->inMode) ||
		(pointToPointMode && pointToPointMode->mInMode) ||
		mInPreviewLoop ||
...
bool EditPolyObject::PrepareDisplay(
...
bool EditPolyObject::UpdatePerNodeItems(
	...
	MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	{
		mm.InvalidateHardwareMesh(1);
		bResult = UpdatePolyObjectPerNodeItemsDisplay(subdivResult, 
	...
	{
		mm.InvalidateHardwareMesh(1);
		bResult = UpdatePolyObjectPerNodeItemsDisplay(mPreviewMesh, 
			updateDisplayContext,
			nodeContext,
			targetRenderItemContainer);
	}
	...
	{
		bResult = PolyObject::UpdatePerNodeItems(updateDisplayContext,
			nodeContext,
			targetRenderItemContainer);
	}
	...
	IPoint2 *p, ViewExp *vpt)
{
	...
	return pMeshToDisplay->select( gw, inode->Mtls(), &hitRegion, flags & (HIT_ABORTONHIT | HIT_MIRROREDTM), inode->NumMtls() );
}
...
int EditPolyObject::HitTest(TimeValue t, INode* inode, int type, int crossing,
	...
	if (!inMayaSelectionMode && (GetSelectMode() == PS_NONE_SEL) && !pointToPointMode->mInMode)
	{
		...
		//clear our preview
		mHighlightedItems.clear();
		bool wasInLoop = mInPreviewLoop;
		mInPreviewLoop = false;
		...
		//adjacent loop preview if the user hovers while holding shift next to a selected element preview a loop through them
		//only supports point hit testing
		if ((GetKeyState(VK_SHIFT) & 0x8000) && (type == HITTYPE_POINT))
		{
			//cannot be in open edge or element mode
			if ((GetCOREInterface()->GetSubObjectLevel() == EP_SL_VERTEX) ||
				(GetCOREInterface()->GetSubObjectLevel() == EP_SL_EDGE) ||
				(GetCOREInterface()->GetSubObjectLevel() == EP_SL_FACE)
				)
			{
				...
				if (GetCOREInterface()->GetSubObjectLevel() == EP_SL_VERTEX)
				{
					hitLevel = SUBHIT_MNVERTS;
					subLevel = MNM_SL_VERTEX;
				}
				else if (GetCOREInterface()->GetSubObjectLevel() == EP_SL_EDGE)
				{
					hitLevel = SUBHIT_MNEDGES;
					subLevel = MNM_SL_EDGE;
				}
				else if (GetCOREInterface()->GetSubObjectLevel() == EP_SL_FACE)
				{
					hitLevel = SUBHIT_MNFACES;
					subLevel = MNM_SL_FACE;
				}

				IMNMeshUtilities13* l_mesh13 = static_cast<IMNMeshUtilities13*>(mm.GetInterface(IMNMESHUTILITIES13_INTERFACE_ID));
				if (l_mesh13)
				{
					//hit test faces first
					mm.SubObjectHitTest(gw, gw->getMaterial(), &hr, SUBHIT_MNFACES | SUBHIT_UNSELONLY, hitList);
					BitArray loop;
					BitArray currentSel;
					//need to hit the mesh
					if (!hitList.IsEmpty())
					{
						//we hit the mesh
						MeshSubHitRec *rec = GetClosestHitRec(hitList);
						int hitFace = rec->index;
						int hitIndex = -1;
						if (hitLevel == SUBHIT_MNFACES)
						{
							//store off the hit face and set our bitarrays to the right size
							hitIndex = hitFace;
							loop.SetSize(mm.numf);
							mm.getFaceSel(currentSel);
						}
						else if (hitLevel == SUBHIT_MNVERTS)
						{
							//store off the hit face and set our bitarrays to the right size
							loop.SetSize(mm.numv);
							mm.getVertexSel(currentSel);
							//check that the vertex in on the hit face to make sure we dont grab an occuluded or back face vert
							mm.SubObjectHitTest(gw, gw->getMaterial(), &hr, hitLevel | SUBHIT_UNSELONLY, hitList);
							if (!hitList.IsEmpty())
							{
								MeshSubHitRec *rec = GetClosestHitRec(hitList);
								int index = rec->index;
								MNFace& f = mm.f[hitFace];
						...
						else if (hitLevel == SUBHIT_MNEDGES)
						{
							//store off the hit face and set our bitarrays to the right size
							loop.SetSize(mm.nume);
							mm.getEdgeSel(currentSel);
							//check that the edge in on the hit face to make sure we dont grab an occuluded or back face vert
							//get the hit edge
							mm.SubObjectHitTest(gw, gw->getMaterial(), &hr, hitLevel | SUBHIT_UNSELONLY, hitList);
							if (!hitList.IsEmpty())
							{
								MeshSubHitRec *rec = GetClosestHitRec(hitList);
								int index = rec->index;
								//edge needs to share with the hit edge
								MNEdge& e = mm.e[index];
								if (e.f1 == hitFace)
									hitIndex = index;
								else if (e.f2 == hitFace)
									hitIndex = index;
							}
						}
						bool isRing = false;
						bool hasSelectedNeighbor = HasAdjacentSelection(&mm, subLevel, hitIndex,true,isRing);
						//we have a neighbor
						if (hasSelectedNeighbor)
						{
							...
							loop.ClearAll();
							loop.Set(hitIndex, TRUE);
							bool foundLoop = false;
							//compute our loops
							if (subLevel == MNM_SL_VERTEX)
							{
								foundLoop = l_mesh13->FindLoopVertex(loop, currentSel);
							}
							else if (subLevel == MNM_SL_EDGE)
							{
								...
								if (isRing)
									mm.SelectEdgeRing(loop);
								else
									mm.SelectEdgeLoop(loop);
								if (loop.NumberSet() > 1)
									foundLoop = TRUE;
							}
							else if (subLevel == MNM_SL_FACE)
							{
								foundLoop = l_mesh13->FindLoopFace(loop, currentSel);
							}
							if (foundLoop)
							{
								//store them off in the selections and previewing
								mInPreviewLoop = true;
								...
								vpt->LogHit(inode, mc, hitIndex, hitIndex, NULL); //need a dummy hit so that we get a selection call on hit

								mSelLevelOverride = subLevel;
								EpHighlightChanged();
								RefreshScreen();
							}
							return 1;
...
void EditPolyObject::DoubleClickSelect(HitRecord *hitRec) {
	...
	// If there are "bad verts", such as verts welded on open edges, which we allow temporarily, the SelectEdgeLoop call can can alter topology underneath us.
	// We use the TemporaryNoBadVerts helper class to temporarily allow bad verts so that can't happen.
	{
		TemporaryNoBadVerts bv(mm);
...
void EditPolyObject::SetupNamedSelDropDown() {
	...
	for (int i=0; i<set.Count(); i++) GetCOREInterface()->AppendSubObjectNamedSelSet(*(set.names[i]));
}
...
void EditPolyObject::UpdateNamedSelDropDown () {
	...
	if (i<ns.Count()) GetCOREInterface()->SetCurNamedSelSet (*(ns.names[i]));
	else GetCOREInterface()->ClearCurNamedSelSet ();
}
	...
	for (int loopCount=0; loopCount<100; loopCount++) {
		...
		if (useDlg) {
			...
			if (!DialogBoxParam (hInstance, MAKEINTRESOURCE(IDD_NAMEDSET_PASTE),
				GetCOREInterface()->GetMAXHWnd(), PickSetNameDlgProc, (LPARAM)&name)) return FALSE;
		} else {
...
int EditPolyObject::SelectNamedSet() {
	...
	return DialogBoxParam (hInstance, MAKEINTRESOURCE(IDD_NAMEDSET_SEL),
		GetCOREInterface()->GetMAXHWnd(), PickSetDlgProc, (LPARAM)&setList);
}
...
public:
	NamedSetSizeIncrease () : mpEditPoly(NULL), nsl(0), oldsize(0), increase(0) { }
...
BOOL EditPolyObject::PlugControl(TimeValue t,int i) {
	...
	{
		// Ordinarily this shouldn't happen -- But out-of-bounds cont[] array access below has triggered CERs so let's play it safe...
...
public:
	Epomop () : mpEditPoly(NULL), testEscape(false), originalCursor(NULL) { }
...
RefResult EditPolyObject::NotifyRefChanged (const Interval& changeInt, RefTargetHandle hTarget,
	PartID& partID, RefMessage message, BOOL propagate) {
		...
		case REFMSG_CHANGE:
			if (hTarget == pblock) {
				...
				case ep_edge_chamfer_quad_intersections:
...
void EditPolyObject::EpSetVertexFlags (BitArray &vset, DWORD flags, DWORD fmask, bool undoable) {
	...
	if (flags & (MN_HIDDEN|MN_SEL)) {
		...
		if (flags & MN_HIDDEN) channels |= GEOM_CHANNEL;	// for bounding box change
...
void EditPolyObject::EpSetEdgeFlags (BitArray &eset, DWORD flags, DWORD fmask, bool undoable) {
	...
	if (flags & MN_SEL) LocalDataChanged(SELECT_CHANNEL|DISP_ATTRIB_CHANNEL);
}
...
public:
	...
	DeltaConstrainer &operator=(const DeltaConstrainer &from) { DbgAssert(0); return *this; }	// Don't need this, but just in case, catch assignments that copy the pointer improperly
};
...
void EditPolyObject::InvalidateTempData (ChannelMask channels) {
	...
	if (channels & GEOM_CHANNEL) mm.InvalidateGeomCache ();
	if ((channels & SELECT_CHANNEL) && tempData) tempData->freeBevelInfo();
...
void EditPolyObject::DragMoveAccept (TimeValue t) {
	...
	if (cont.Count()) {
		for (int i=0; i<tempMove->active.GetSize(); i++) {
			...
			if ((i < cont.Count()) && cont[i]) cont[i]->SetValue (t, &(mm.v[i].p));
		}
	}
...
public:
	...
	void AddFaces(const BitArray &faces);	// Adds the faces in the list if they aren't already stored, with their smoothing groups
...
HardSmoothRestore::HardSmoothRestore(EditPolyObject *e) : epo(e) {
...
void HardSmoothRestore::AddFace(int index) {
	...
	if(undoFaces[index] == 0) {
		...
		afterSmooth.Append(1, &theFace.smGroup, 10);	// Use this one temporarily to reserve a spot for it, change it in the After() method
	}
}
...
void HardSmoothRestore::Restore(int /*isUndo*/) {
	if(undoFaceIndexes.Count() == 0) return;
	for(int i = 0; i < undoFaceIndexes.Count(); ++i) {
		int theFaceIndex = undoFaceIndexes[i];
		MNFace &theFace = epo->mm.f[theFaceIndex];
...
int HardSmoothRestore::Size() {
	...
	return sizeof(void *) + sizeof(BitArray) + (count + 7) / 8 + sizeof(IntTab) + count * sizeof(int) + sizeof(DWORDTab) * 2 + count * sizeof(DWORD) * 2;
}

// Note: this code is duplicated in Edit Poly (minus the undo system calls); Be sure to make any changes there when changing this code
// See maxsdk\samples\mesh\EditPoly\EditPolyOps.cpp, PolyOpMakeHardEdges::Do
...
bool EditPolyObject::EpfnMakeHardEdges (DWORD flag) {
	...
	for(int i = 0; i < mm.nume; ++i) {
		...
		for(int face = 0; face < 2; ++face) {
			...
			mm.FaceFindAdjacentFaces(faceIndex, surroundingFaces, surroundingFaceIndexes, false);		// Don't include the root face (false)
			...
			for(int j = 0; j < theEdgeFace.deg; ++j) {
				...
				mm.FaceFindAdjacentFaces(otherFaceIndex, adjacentFaces, adjacentFaceIndexes, true);	// Include the other face itself
...
// Note: this code is duplicated in Edit Poly (minus the undo system calls); Be sure to make any changes there when changing this code
// See maxsdk\samples\mesh\EditPoly\EditPolyOps.cpp, PolyOpMakeSmoothEdges::Do
