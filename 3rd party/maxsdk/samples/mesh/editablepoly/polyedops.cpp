...
void EditPolyObject::EpActionButtonOp (int opcode) {
	...
	case epop_create_shape:
		...
		if (i>= mm.nume) {
			...
			MessageBox (GetCOREInterface()->GetMAXHWnd(),buf2,buf1,MB_ICONEXCLAMATION|MB_OK);
			break;
		}
...
bool EditPolyObject::EpfnChamferMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	if (msl == MNM_SL_EDGE)
		pblock->GetValue (ep_edge_chamfer_type, TimeValue(0), chamferType, FOREVER);
	if (chamferType == EP_QUAD_EDGE_CHAMFER) return true;
	...
	float chamfer = 0.0f;
	int		l_smooth = 0;
	int		l_smooth_type = EP_EDGE_CHAMFER_SMOOTH_ALL;
	...
	case MNM_SL_EDGE:
		...
		pblock->GetValue (ep_edge_chamfer_smooth, TimeValue(0), l_smooth, FOREVER);
		pblock->GetValue (ep_edge_chamfer_smooth_type, TimeValue(0), l_smooth_type, FOREVER);
		pblock->GetValue (ep_edge_chamfer_smooth_threshold, TimeValue(0), l_smooth_threshold, FOREVER);
		break;
	}
...
bool EditPolyObject::DoExtrusionMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag, bool isBevel) {
	...
	if (isBevel) {
		pblock->GetValue(ep_bevel_type, TimeValue(0), extType, FOREVER);
		pblock->GetValue(ep_bevel_bias, TimeValue(0), bias, FOREVER);
	}
	else {
		pblock->GetValue(ep_extrusion_type, TimeValue(0), extType, FOREVER);
		pblock->GetValue(ep_extrude_bias, TimeValue(0), bias, FOREVER);
	}
	...
	case 1:	// Local normals
        {
            ...
            if (ret)
            {
                ...
                l_mesh16->GetExtrudeDirection(temp->ChamferData(), flag, bias);
            }
        }
		break;
...
bool EditPolyObject::DoChamferMesh (MNMesh & mesh, MNTempData *temp, int msl, DWORD flag) {
	...
	bool	ret		= false;
	DWORD	l_flag	= MN_SEL;
	int		l_type	= EP_STANDARD_EDGE_CHAMFER;
	float	l_amount = 0.0f;
	...
	case MNM_SL_VERTEX:
		...
		break;
	case MNM_SL_EDGE:
		pblock->GetValue (ep_edge_chamfer_type, TimeValue(0), l_type, FOREVER);
		pblock->GetValue (ep_edge_chamfer, TimeValue(0), l_amount, FOREVER);
		pblock->GetValue (ep_edge_chamfer_tension, TimeValue(0), l_tension, FOREVER);
		pblock->GetValue (ep_edge_chamfer_open, TimeValue(0), l_open, FOREVER);
		pblock->GetValue (ep_edge_chamfer_invert, TimeValue(0), l_invert, FOREVER);
		pblock->GetValue (ep_edge_chamfer_segments, TimeValue(0), l_edgeSegments, FOREVER);
		pblock->GetValue (ep_edge_chamfer_smooth, TimeValue(0), l_smooth, FOREVER);
		pblock->GetValue (ep_edge_chamfer_smooth_type, TimeValue(0), l_smooth_type, FOREVER);
		pblock->GetValue (ep_edge_chamfer_smooth_threshold, TimeValue(0), l_smooth_threshold, FOREVER);
		pblock->GetValue (ep_edge_chamfer_quad_intersections, TimeValue(0), l_quad_intersections, FOREVER);
		break;
	}
	...
	case MNM_SL_EDGE:
		{
			...
			case EP_STANDARD_EDGE_CHAMFER: {
				...
				// We don't perform the smoothing operation (if any) at this time because the above operation just
				// prepared the deltas for the operation.  They're actually applied in a second step, then we smooth
											} break;
			case EP_QUAD_EDGE_CHAMFER: {
				MN_QCHAM_TYPE result = l_bOpen ? ( l_bInvert ? QCHAM_CHAMFERSONLY : QCHAM_NOCHAMFERS) : QCHAM_CHAMFEREDOBJECT;
				DWORD options = l_quad_intersections ? QCHAM_OPTION_QUAD_INTERSECTIONS : QCHAM_OPTION_ELWOOD_DEFAULTS;
				mesh.QuadChamfer(l_amount, l_edgeSegments, l_tension, options, result, MN_SEL, QCHAM_VERSION_LATEST);
				// If we're smoothing, take care of that here.
				if(l_smooth) {
					switch(l_smooth_type) {
					case EP_EDGE_CHAMFER_SMOOTH_ALL:
...
void EditPolyObject::EpfnBeginChamfer (int msl, TimeValue t) {
	...
	if(msl == MNM_SL_EDGE)
		pblock->GetValue (ep_edge_chamfer_type, TimeValue(0), chamferType, FOREVER);
	if (!DoChamfer (msl, MN_SEL) || (chamferType == EP_STANDARD_EDGE_CHAMFER && !TempData()->ChamferData()->vmax.Count())) {
		theHold.SuperCancel ();
		return;
	}
	BitArray sel;
...
void EditPolyObject::EpfnEndChamfer (bool accept, TimeValue t) {
	if (!inChamfer) return;
	inChamfer = FALSE;
	// Two different chamfer types -- standard and quad
	int chamferType = EP_STANDARD_EDGE_CHAMFER;
	if(GetMNSelLevel() == MNM_SL_EDGE)
		pblock->GetValue (ep_edge_chamfer_type, TimeValue(0), chamferType, FOREVER);
	if(chamferType == EP_STANDARD_EDGE_CHAMFER) {     // Standard chamfer
		TempData()->freeChamferData();
...
void EditPolyObject::EpfnDragChamfer (float amount, float tension, TimeValue t) {
	if (!inChamfer) return;
	...
	if(chamferType == EP_STANDARD_EDGE_CHAMFER) {     // Standard chamfer
		...
		TopoChangeRestore *theTopoChange = theTopoChangeRestore.Get();
		DbgAssert(theTopoChange != NULL);
		theTopoChange->Restore(false);			// Put everything back where it was before doing this iteration
		DoChamferMesh(mm, NULL, MNM_SL_EDGE);
	}
}
