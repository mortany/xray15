...
bool BevelGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eBevelOutline:
		...
		break;
	case eBevelBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_bevel_bias, t, value.mFloat, FOREVER);
		break;
...
bool ExtrudeFaceGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeBias:
		GetEditPolyObj()->getParamBlock()->GetValue(ep_extrude_bias, t, value.mFloat, FOREVER);
		break;
	default:
		return false;
	}
...
bool ChamferEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eChamferEdgeType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_type, t, value.mInt,FOREVER);
		break;
	case eChamferEdgeAmount:
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_tension, t, value.mFloat,FOREVER);
		break;
	case eChamferEdgeOpen:
		{
			int val = 0 ;
	...
	case eChamferEdgeInvert:
		{
			int val = 0 ;
			GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_invert, t, val,FOREVER);
			value.mbool = ( val == 0 ? false:true );
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			int val = 0;
			GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_quad_intersections, t, val, FOREVER);
			value.mbool = (val == 0 ? false : true);
		}
		break;
	case eChamferEdgeSmooth:
		{
            ...
            int val = 0;
		    GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_smooth, t, val,FOREVER);
			value.mbool = (val == 0 ? false: true);
		}
		break;
	case eChamferEdgeSmoothType:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_smooth_type, t, value.mInt,FOREVER);
		break;
	case eChamferEdgeSmoothThreshold:
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_smooth_threshold, t, value.mFloat,FOREVER);
		break;
	default:
		return false;
	}
...
void ChamferEdgeGrip::MaybeEnableControls() {
	...
	int chamferType;
	GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_type, 0, chamferType,FOREVER);
	if( chamferType == EP_QUAD_EDGE_CHAMFER)	// Quad
	{
		...
		int openOn;
		GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_open, 0, openOn,FOREVER);
	...
	int smoothingOn;
	GetEditPolyObj()->getParamBlock()->GetValue (ep_edge_chamfer_smooth, 0, smoothingOn,FOREVER);
...
bool ChamferEdgeGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eChamferEdgeType:
		...
		GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_type, t, value.mInt);
		MaybeEnableControls();
		break;
	case eChamferEdgeAmount:
		if( value.mFloat < 0.0f )
	...
	case eChamferEdgeQuadIntersections:
		{
			int val = (int)value.mbool;
			GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_quad_intersections, t, val);
			GetEditPolyObj()->getParamBlock()->GetValue(ep_edge_chamfer_quad_intersections, t, val, FOREVER);
		}
		break;
	case eChamferEdgeSmooth:
...
bool ChamferEdgeGrip::ShowKeyBrackets(int which,TimeValue t)
{
	...
	case eChamferEdgeTension:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_tension,t)==TRUE) ? true : false;
	case eChamferEdgeOpen:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_open,t)==TRUE) ? true : false;
	case eChamferEdgeInvert:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_invert,t)==TRUE) ? true : false;
	case eChamferEdgeSmoothThreshold:
		return (GetEditPolyObj()->getParamBlock()->KeyFrameAtTimeByID(ep_edge_chamfer_smooth_threshold, t) == TRUE) ? true : false;
	default:
		return false;
	}
...
{
	const int iResetSegVal = 1;
	const int iResetSmoothVal = 0;
	const int iResetSmoothTypeVal = EP_EDGE_CHAMFER_SMOOTH_ALL;
	const float fResetVal = 0.0f;
	const float fResetTensionVal = 1.0f;
	const float fResetSmoothThresholdVal = 30.0f;
	...
	case eChamferEdgeType:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_type, t, EP_STANDARD_EDGE_CHAMFER);	// resets to standard chamfer
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeAmount:
	...
	case eChamferEdgeTension:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_tension, t, fResetTensionVal);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeOpen:
	...
	case eChamferEdgeInvert:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_invert, t, 0);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			theHold.Begin();
			GetEditPolyObj()->getParamBlock()->SetValue(ep_edge_chamfer_quad_intersections, t, 1);	// resets to improved style
			theHold.Accept(GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmooth:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_smooth, t, CHAMFER_EDGE_SMOOTH_ON);				// Resets to smoothing on
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmoothType:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_smooth_type, t, EP_EDGE_CHAMFER_SMOOTH_ALL);	// resets to smooth all
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmoothThreshold:
		{
			theHold.Begin ();
			GetEditPolyObj()->getParamBlock()->SetValue (ep_edge_chamfer_smooth_threshold, t, 30.0f);			// Resets to 30 degree threshold
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	default:
...
bool ChamferEdgeGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eChamferEdgeType:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_type];
	...
	case eChamferEdgeTension:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_tension];
			resetValue.mFloat = pd.def.f;
		}
		break;
	case eChamferEdgeOpen:
	...
	case eChamferEdgeInvert:
		{ 
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_invert];
			resetValue.mbool = (pd.def.i == 0 ? false:true);
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_quad_intersections];
			resetValue.mbool = (pd.def.i == 0 ? false : true);
		}
		break;
	case eChamferEdgeSmooth:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_smooth];
			resetValue.mbool = (pd.def.i == 0 ? false:true) ;
		}
		break;
	case eChamferEdgeSmoothType:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_smooth_type];
			resetValue.mInt = pd.def.i;
		}
		break;
	case eChamferEdgeSmoothThreshold:
		{
			const ParamDef& pd = pbd->paramdefs[ep_edge_chamfer_smooth_threshold];
			resetValue.mFloat = pd.def.f;
		}
		break;
	default:
