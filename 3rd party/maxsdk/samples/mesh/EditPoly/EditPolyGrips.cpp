	...
	case eSetFlow:
		{
			...
			ExecuteMAXScriptScript(strScript.data(), TRUE); //this is from PolyTools.ms, SetFlowSpinnerCallback
			value.mInt = 0; //keep the value as zero for the spinner.
		}
		break;
	case eLoopShift:
...
bool BevelGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eBevelOutline:
		...
		break;
	case eBevelBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_face_bias, t, value.mFloat, FOREVER);
		break;
	default:
		return false;
	}
...
bool BevelGrip::ResetValue(TimeValue t, int which)
{
	...
	case eBevelBias:
	{
		...
		GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_bevel_face_bias, t, fResetVal, FOREVER);
		theHold.Accept(GetString(IDS_BEVEL));
...
bool ExtrudeFaceGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eExtrudeFaceHeight:
		...
		break;
	case eExtrudeFaceBias:
		GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_bias, t, value.mFloat, FOREVER);
		break;
	default:
		return false;
	}
...
bool ExtrudeFaceGrip::ResetValue(TimeValue t, int which)
{
	...
	case eExtrudeFaceBias:
	{
		...
		GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
		GetEditPolyMod()->getParamBlock()->GetValue(epm_extrude_face_bias, t, fResetVal, FOREVER);
		theHold.Accept(GetString(IDS_EXTRUDE));
	}
	break;
	default:
...
bool ChamferEdgeGrip::GetText(int which,TSTR &string)
{
	...
	case eChamferEdgeType:
		{
			string =GetString(IDS_EDGE_CHAMFER_TYPE);
		}
		break;
	case eChamferEdgeAmount:
		{
	...
	case eChamferEdgeQuadIntersections:
		{
			string = GetString(IDS_EDGE_CHAMFER_QUAD_INTERSECTIONS);
		}
		break;
	case eChamferEdgeSmooth:
		{
...
bool ChamferEdgeGrip::GetResolvedIconName(int which,MSTR &string)
{
	...
	case eChamferEdgeInvert:
		...
		break;
	case eChamferEdgeQuadIntersections:
		string = CHAMFER_EDGE_QUAD_INTERSECTIONS_PNG;
		break;
	case eChamferEdgeSmooth:
...
{
	if(which== eChamferEdgeType)
	{
		IBaseGrip::ComboLabel *label = new IBaseGrip::ComboLabel();
		TSTR string = CHAMFER_EDGE_STANDARD_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_STANDARD_CHAMFER);
		comboOptions.Append(1,&label);
		label = new IBaseGrip::ComboLabel();
		string = CHAMFER_EDGE_QUAD_PNG;
		label->mIcon = string;
		label->mLabel = GetString(IDS_QUAD_CHAMFER);
		comboOptions.Append(1,&label);
		return true;
	}
	if(which== eChamferEdgeSmoothType)
	{
		IBaseGrip::ComboLabel *label = new IBaseGrip::ComboLabel();
		TSTR string = CHAMFER_EDGE_SMOOTH_ALL_PNG;
		label->mIcon = string;
...
bool ChamferEdgeGrip::GetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	{
	case eChamferEdgeType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_type, t, value.mInt,FOREVER);
		break;
	case eChamferEdgeAmount:
	...
	case eChamferEdgeSegments:
		...
		break;
	case eChamferEdgeTension:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_tension, t, value.mFloat,FOREVER);
		break;
	case eChamferEdgeOpen:
		{
			// need to go BOOL(int) to bool
	...
	case eChamferEdgeInvert:
		{
			int val = 0 ;
			GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_invert, t, val,FOREVER);
			value.mbool = ( val == 0 ? false:true );
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			int val = 0;
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_quad_intersections, t, val, FOREVER);
			value.mbool = (val == 0 ? false : true);
		}
		break;
	case eChamferEdgeSmooth:
		{
			// need to go BOOL(int) to bool
            int val = 0;
		    GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth, t, val,FOREVER);
			value.mbool = (val == 0 ? false: true);
		}
		break;
	case eChamferEdgeSmoothType:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_type, t, value.mInt,FOREVER);
		break;
	case eChamferEdgeSmoothThreshold:
		GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_threshold, t, value.mFloat,FOREVER);
		break;
	default:
		return false;
	}
...
void ChamferEdgeGrip::MaybeEnableControls() {
	...
	int chamferType;
	GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_type, 0, chamferType,FOREVER);
	if( chamferType == EP_QUAD_EDGE_CHAMFER)	// Quad
	{
		gripManager->ActivateGripItem(eChamferEdgeTension, true);
		gripManager->ActivateGripItem(eChamferEdgeQuadIntersections, true);
		// Only enable Open|Invert option if "Open" is turned on
		int openOn;
		GetEditPolyMod()->getParamBlock()->GetValue (epm_chamfer_edge_open, 0, openOn,FOREVER);
		gripManager->ActivateGripItem(eChamferEdgeInvert,openOn ? true : false);
	}
	else
	{
		// Standard chamfer doesn't have these options
		gripManager->ActivateGripItem(eChamferEdgeTension,false);
		gripManager->ActivateGripItem(eChamferEdgeInvert,false);
		gripManager->ActivateGripItem(eChamferEdgeQuadIntersections, false);
	}

	// Only enable additional smoothing options if smoothing is turned on
	int smoothingOn;
	GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth, 0, smoothingOn,FOREVER);
...
bool ChamferEdgeGrip::SetValue(int which,TimeValue t,IBaseGrip::GripValue &value)
{
	...
	case eChamferEdgeType:
		{
		if(value.mInt>EP_QUAD_EDGE_CHAMFER) //go back to zero
			value.mInt =EP_STANDARD_EDGE_CHAMFER;
		if(value.mInt<EP_STANDARD_EDGE_CHAMFER)
			value.mInt = EP_QUAD_EDGE_CHAMFER;
		GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_type, t, value.mInt);
	...
	case eChamferEdgeQuadIntersections:
		{
			int val = (int)value.mbool;
			GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_quad_intersections, t, val);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_quad_intersections, t, val, FOREVER);
		}
		break;
	case eChamferEdgeSmooth:
...
bool ChamferEdgeGrip::ResetValue(TimeValue t, int which)
{
    ...
    int iResetSegVal = 1;
	int iResetVal = 0;
	float fResetTensionVal = 1.0f;
	float fResetSmoothThresholdVal = 30.0f;
	...
	case eChamferEdgeType:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_type, t, EP_STANDARD_EDGE_CHAMFER);	// resets to standard chamfer
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_type, t, iResetVal, FOREVER);	// resets to standard chamfer
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeAmount:
	...
	case eChamferEdgeTension:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_tension, t, fResetTensionVal);
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_tension, t, fResetTensionVal,FOREVER);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeOpen:
	...
	case eChamferEdgeInvert:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_invert, t, iResetVal);
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_invert,t,iResetVal,FOREVER);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			theHold.Begin();
			GetEditPolyMod()->getParamBlock()->SetValue(epm_edge_chamfer_quad_intersections, t, iResetVal);
			GetCOREInterface()->RedrawViews(GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_quad_intersections, t, iResetVal, FOREVER);
			theHold.Accept(GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmooth:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth, t, CHAMFER_EDGE_SMOOTH_ON);	// Resets to smoothing on
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_smooth,t,iResetVal,FOREVER);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmoothType:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth_type, t, EP_EDGE_CHAMFER_SMOOTH_ALL);	// resets to smooth all
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue (epm_edge_chamfer_smooth_type, t, iResetVal, FOREVER);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	case eChamferEdgeSmoothThreshold:
		{
			theHold.Begin ();
			GetEditPolyMod()->getParamBlock()->SetValue (epm_edge_chamfer_smooth_threshold, t, 30.0f);	// Resets to 30 degree threshold
			GetCOREInterface()->RedrawViews (GetCOREInterface()->GetTime(), REDRAW_INTERACTIVE);
			GetEditPolyMod()->getParamBlock()->GetValue(epm_edge_chamfer_smooth_threshold, t, fResetSmoothThresholdVal,FOREVER);
			theHold.Accept (GetString(IDS_CHAMFER));
		}
		break;
	default:
...
bool ChamferEdgeGrip::GetResetValue(int which,IBaseGrip::GripValue &resetValue)
{
	...
	case eChamferEdgeType:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_type];
			resetValue.mInt = pd.def.i;
		}
		break;
	case eChamferEdgeAmount:
	...
	case eChamferEdgeTension:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_tension];
			resetValue.mFloat = pd.def.f;
		}
		break;
	case eChamferEdgeOpen:
	...
	case eChamferEdgeInvert:
		{ 
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_invert];
			resetValue.mbool = (pd.def.i == 0 ? false:true);
		}
		break;
	case eChamferEdgeQuadIntersections:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_quad_intersections];
			resetValue.mbool = (pd.def.i == 0 ? false : true);
		}
		break;
	case eChamferEdgeSmooth:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_smooth];
			resetValue.mbool = (pd.def.i == 0 ? false:true) ;
		}
		break;
	case eChamferEdgeSmoothType:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_smooth_type];
			resetValue.mInt = pd.def.i;
		}
		break;
	case eChamferEdgeSmoothThreshold:
		{
			const ParamDef& pd = pbd->paramdefs[epm_edge_chamfer_smooth_threshold];
			resetValue.mFloat = pd.def.f;
		}
		break;
	default:
