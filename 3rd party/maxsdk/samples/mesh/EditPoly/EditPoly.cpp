...
void EditPolyMod::ModifyObject (TimeValue t, ModContext &mc, ObjectState *os, INode *node) {
   ...
   pData->SetOutputCache (mesh);
	...
	//this section deals with EditPoly above and OSD and losing the OSD Display interface in some cases due to how caching works
	//get our OSD display interfaces on the in and out meshes
	iMNMESH_GPU_Display *gpu_MNMeshIn_Display = (iMNMESH_GPU_Display *) mesh.GetInterface(IMNMESH_GPU_DISPLAY_INTERFACE); //store off the incoming OSD GPU display interface
	iMNMESH_GPU_Display *gpu_MNMeshOut_Display = (iMNMESH_GPU_Display *) pObj->GetMesh().GetInterface(IMNMESH_GPU_DISPLAY_INTERFACE);

	if (gpu_MNMeshIn_Display && gpu_MNMeshOut_Display)  //meshes flowing up the stack get deleted willy nilly so need a more permanent place to store ie the local data
	{
		...
		{		   
			pData->mOSD_GPU_Display->mpCustomRenderItem = gpu_MNMeshIn_Display->mpCustomRenderItem;  //this is an autoptr so dont need to deal with ownership
		}
...
// we need to modify the closest vert in case it shares the edge...
   ...
   if(hitList&&selOnly&&leftButtonDown)  //fix for 924888
   {
	   ...
	   switch(mSelLevelOverride)
	   {
		...
		MeshSubHitRec *rec = &(*hitList->begin());
		if(rec)
		{
   ...
   if (!inMayaSelectionMode && (GetSelectMode() == PS_NONE_SEL) && !pointToPointMode->mInMode)
   {
	   ...
	   if (wasInLoop)
	   {
		   if (!(GetKeyState(VK_SHIFT) & 0x8000))
			pData->mShiftSelMesh.ClearAndFree();
		   EpModHighlightChanged();
		   EpModRefreshScreen();
	   }
	   ...
	   if ((GetKeyState(VK_SHIFT) & 0x8000) && (type == HITTYPE_POINT))
	   {
		   ...
		   if ((GetCOREInterface()->GetSubObjectLevel() == EPM_SL_VERTEX) ||
			   ...
			   (GetCOREInterface()->GetSubObjectLevel() == EPM_SL_FACE)
			   )
		   {
			   if ((pData->mShiftSelMesh.numf == 0) || (pData->mShiftSelMesh.numf != pMesh->numf))
				   pData->mShiftSelMesh = *pMesh;
			   else
			   {
				   //sync our selections
				   BitArray sel;
				   pMesh->getVertexSel(sel);
				   pData->mShiftSelMesh.VertexSelect(sel);
				   pMesh->getEdgeSel(sel);
				   pData->mShiftSelMesh.EdgeSelect(sel);
				   pMesh->getFaceSel(sel);
				   pData->mShiftSelMesh.FaceSelect(sel);
			   }
			   SubObjHitList hitList;
			   DWORD hitLevel = 0;
			   DWORD subLevel = 0;
			   ...
			   IMNMeshUtilities13* l_mesh13 = static_cast<IMNMeshUtilities13*>(pMesh->GetInterface(IMNMESHUTILITIES13_INTERFACE_ID));
			   if (l_mesh13)
			   {
				   gw->setRndLimits(gw->getRndLimits() | GW_BACKCULL);
				   //hit test faces first
				   if (hitLevel == SUBHIT_MNFACES)
				   pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, SUBHIT_MNFACES | SUBHIT_UNSELONLY, hitList);
				   else  //if we are not in face mode we want to hit test all faces since the hit face is just used for occulusion testing
					   pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, SUBHIT_MNFACES, hitList);
				   BitArray loop;
				   BitArray currentSel;
				   ...
				   if (!hitList.IsEmpty())
				   {
					   ...
					   if (hitLevel == SUBHIT_MNFACES)
					   {
						   ...
						   hitIndex = hitFace;
						   loop.SetSize(pMesh->numf);
						   pMesh->getFaceSel(currentSel);
					   }
					   else if (hitLevel == SUBHIT_MNVERTS)
					   {
						   //store off the hit face and set our bitarrays to the right size
						   loop.SetSize(pMesh->numv);
						   pMesh->getVertexSel(currentSel);
						   //check that the vertex in on the hit face to make sure we dont grab an occuluded or back face vert
						   pMesh->SubObjectHitTest(gw, gw->getMaterial(), &hr, hitLevel | SUBHIT_UNSELONLY, hitList);
						   if (!hitList.IsEmpty())
						   {
							   MeshSubHitRec *rec = GetClosestHitRec(hitList);
							   int index = rec->index;
							   MNFace& f = pMesh->f[hitFace];
					   ...
					   {
						   //store off the hit face and set our bitarrays to the right size
						   loop.SetSize(pMesh->nume);
						   pMesh->getEdgeSel(currentSel);
					   ...
					   bool isRing = false;
					   bool hasSelectedNeighbor = HasAdjacentSelection(pMesh, subLevel, hitIndex, true, isRing);
					   //we have a neighbor
					   if (hasSelectedNeighbor)
					   {
						   ...
						   {
							   TemporaryNoBadVerts nbv(*pMesh);
							   if (isRing)
								   pMesh->SelectEdgeRing(loop);
							   else
								   pMesh->SelectEdgeLoop(loop);
							   if (loop.NumberSet() > 1)
								   foundLoop = TRUE;
						   }
...
void EditPolyMod::DoubleClickSelect(HitRecord *hitRec)
{
	...
	if (!pData->GetNewSelection ()) {
		...
		{
			TemporaryNoBadVerts nbv(*(pData->GetMesh()));
...
void EditPolyMod::SelectSubComponent (HitRecord *pFirstHit, BOOL selected, BOOL all, BOOL invert)
{
   ...
   if (mSelLevelOverride>0) //special multi or highlight selection
   {
	   ...
	   if (mInPreviewLoop)
	   {
		   if (pData->GetMeshOutput())
			pData->mShiftSelMesh = *pData->GetMeshOutput();
	   }
...
IOResult EditPolyMod::LoadLocalData(ILoad *iload, LocalModData **pld) {
   ...
   *pld = pData;
   ...
   // The "bad vertex" removal fix went in after build 1167 of 2019 Update1, so anything saved earlier needs to be tagged.
...
bool EditPolyMod::ManipGripIsValid()
{
	...
	if( GetEPolySelLevel() == EPM_SL_EDGE) // only valid for edge modes.
	{
		...
		GetManipulateGrip( EPolyMod13::eEdgeCrease) || 
		GetManipulateGrip( EPolyMod13::eEdgeWeight))
		return true;
	}
	if( GetEPolySelLevel() == EPM_SL_VERTEX) // only valid for vertex modes.
	{
		if( GetManipulateGrip( EPolyMod13::eVertexCrease) || 
		GetManipulateGrip( EPolyMod13::eVertexWeight))
		return true;
	}
	return false;
}
...
void EditPolyMod::SetUpManipGripVisibility()
{
	...
	{
		BitArray copy;
		copy.SetSize((int)EPolyMod13::EditPolyManipulatorGripCount);
		...
		copy.Set((int)EPolyMod13::eEdgeCrease, GetManipulateGrip(EPolyMod13::eEdgeCrease)) ;
		copy.Set((int)EPolyMod13::eEdgeWeight, GetManipulateGrip(EPolyMod13::eEdgeWeight)) ;
		copy.Set((int)EPolyMod13::eVertexCrease, GetManipulateGrip(EPolyMod13::eVertexCrease)) ;
		copy.Set((int)EPolyMod13::eVertexWeight, GetManipulateGrip(EPolyMod13::eVertexWeight)) ;
		if( GetEPolySelLevel() != EPM_SL_EDGE) // if not in edge, turn off the edge ones!
		{
			...
			copy.Clear((int) EPolyMod13::eEdgeCrease);
			copy.Clear((int) EPolyMod13::eEdgeWeight);
		}
		if( GetEPolySelLevel() != EPM_SL_VERTEX) // if not in vertex, turn off the vertex ones!
		{
			copy.Clear((int) EPolyMod13::eVertexCrease);
			copy.Clear((int) EPolyMod13::eVertexWeight);
		}
		int useSoftsel=0;
		mpParams->GetValue (epm_ss_use, TimeValue(0), useSoftsel, FOREVER);
		if(useSoftsel ==0)
