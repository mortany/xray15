...
namespace
{
	//I know codapge - locale is not one to one corresponding relationship, but no choice
	//better to write the locale info in the scene file in the future.
	TCHAR* GetLocaleFromCodePage(UINT codePage)
	{
		switch(codePage)
		{
...
bool TextObject::ConvertStringToGlyphIndices()
{
	...
	TCHAR* curtLocale = GetLocaleFromCodePage(stringCodePage);
	_tsetlocale(LC_ALL, curtLocale); //Set the new locale
...
IOResult TextObject::Save(ISave *isave) {
	...
	if (saveAsMBCS)
	{
		...
		if (chShapesMBCS.Count() == 0 && !fontHandle)
			ChangeFont(fontName, FontFlagsFromTextFlags());

		ConvertUnicodeToHexStringAndShapes(string2, stringA, isave->CodePage(), chShapesMBCS, fontHandle, textVersion);

		nbytes = (int)stringA.Length() + 1;
		isave->Write(&nbytes, sizeof(int), &nb);
