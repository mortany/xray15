...
ZIPSetupDecode(TIFF* tif)
{
	...
	static const TCHAR module[] = _T("ZIPSetupDecode");

	assert(sp != NULL);
	if (inflateInit(&sp->stream) != Z_OK) {
		TIFFError(module, _T("%s: %hs"), tif->tif_name, sp->stream.msg);
		return (0);
	} else {
...
{
	ZIPState* sp = DecoderState(tif);
	static const TCHAR module[] = _T("ZIPDecode");
	...
	do {
		...
		if (state == Z_DATA_ERROR) {
			TIFFError(module,
			    GetString(IDS_ZIP_DECODE_ERR),//"%s: Decoding error at scanline %d, %hs",
		...
		if (state != Z_OK) {
			TIFFError(module, GetString(IDS_ZLIB_ERR),//"%s: zlib error: %hs",
...
ZIPSetupEncode(TIFF* tif)
{
	...
	static const TCHAR module[] = _T("ZIPSetupEncode");
...
{
	ZIPState *sp = EncoderState(tif);
	static const TCHAR module[] = _T("ZIPEncode");
	...
	do {
		if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
			TIFFError(module, GetString(IDS_ZIP_ENCOD_ERR),//"%s: Encoder error: %hs",
			    tif->tif_name, sp->stream.msg);
			return (0);
		}
		if (sp->stream.avail_out == 0) {
...
{
	ZIPState *sp = EncoderState(tif);
	static const TCHAR module[] = _T("ZIPPostEncode");
	int state;
...
{
	ZIPState* sp = ZState(tif);
	static const TCHAR module[] = _T("ZIPVSetField");
...
TIFFInitZIP(TIFF* tif, int scheme)
{
	...
	return (1);
bad:
	TIFFError(_T("TIFFInitZIP"), GetString(IDS_ZIP_NO_SPACE));//"No space for ZIP state block");
	return (0);
}
#endif /* ZIP_SUPORT */
