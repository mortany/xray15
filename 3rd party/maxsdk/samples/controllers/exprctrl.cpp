...
public:
	...
	virtual void GetClassName(MSTR& s) { s = _M("ExprControl"); } // from Animatable
...
static int updateExpr(HWND hDlg, ExprControl *ec, bool dismiss)
{
	...
	if(ec->expr.load(expr)) {
		if(isEmpty(expr)) {
			...
			int id = MessageBox(hDlg, msg, GetString(IDS_DB_EXPR_PARSE_ERROR),
...
static BOOL BrowseForExprFilename(TCHAR *name, TCHAR *dir, int save) 
{
	...
	ofn.Flags = OFN_HIDEREADONLY | OFN_EXPLORER | OFN_ENABLEHOOK | OFN_ENABLESIZING |(save ? OFN_OVERWRITEPROMPT : (OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST));
...
static void LoadExpression(HWND hDlg, ExprControl *ec)
{
	...
	if(!exprDir[0])
		_tcscpy(exprDir, ec->ip->GetDir(APP_EXPRESSION_DIR));
	TCHAR filename[MAX_PATH] = {0};
	if(BrowseForExprFilename(filename, exprDir, FALSE))	{
		...
		case IDC_CREATE_VAR:
			{
				...
				if(!varName.isNull()) {
					if(!isVariableNameValid(varName)) {
						...
						MessageBox(hDlg, msg, GetString(IDS_DB_CANT_CREATE_VAR), 
							MB_ICONEXCLAMATION | MB_APPLMODAL | MB_OK);
						break;
					}
					theHold.Begin();
		...
		case IDC_DELETE_VAR:
			{
				...
				if(!varName.isNull()) {
					...
					MessageBox(hDlg, msg, GetString(IDS_DB_EXPRCNTL), 
						MB_ICONEXCLAMATION | MB_APPLMODAL | MB_OK);
				}
				else {
