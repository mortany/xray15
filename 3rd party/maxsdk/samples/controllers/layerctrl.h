...
private:	//this is actually set and stored from trackview.
	...
	// we don't check if the nodes are monitored by any of the MasterLayerControls,simply enum all anims of the input nodes
...
class MasterLayerControl : public Control {		
	public:
		...
		void AddLayerControl(LayerControl* lC); //it only append if LC is not in this master layer control.
...
public:
	...
	/*! \internal use!*/
	//! For XRef extension when updating the source controller animation layer by using the animation layer in the master scene.
	//! \param[in] name the animation layer name in the master scene.
...
class LayerControl : public Control, public ILayerControlInternal, public ILockedTrackImp
{
	...
	public:
		...
		Tab<bool> mbInternalFrozenFlags; //record internal frozen flag of the master.
