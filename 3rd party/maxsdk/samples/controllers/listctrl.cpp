...
class ListControlDlg : public ReferenceMaker, public TimeChangeCallback {
	public:
		...
		virtual void GetClassName(MSTR& s) { s = _M("ListControlDlg"); } // from Animatable
...
class CutRestore : public RestoreObj, public ReferenceMaker {	
	public:	
		...
		virtual void GetClassName(MSTR& s) { s = _M("CutRestore"); } // from Animatable
...
RefResult ListControl::NotifyRefChanged( const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate )
	{
	switch (message) {
	...
	if (cont->dlg == this) {
		...
		if (c->conts.Count() > 0) {
			if (c->active < 0)
				c->active = 0;
			if (c->conts[c->active]) {
				c->conts[c->active]->BeginEditParams(
					i, c->paramFlags, NULL);
			}
		}
	}
...
void ListControlDlg::StartEdit(int a)
	{
	if (cont->dlg) {
		if (cont->conts.Count() && cont->conts[a]) {
...
static INT_PTR CALLBACK ListControlDlgProc(
   ...
   static BOOL undoHeldHere = FALSE; //AF -- this flag is used to trigger an undo begin in the even of a WM_CUSTEDIT_ENTER
	...
	switch (message) {
		...
		case WM_DESTROY:						
			delete ld;
			DLSetWindowLongPtr<ListControlDlg*>(hWnd, nullptr);
			break;
