...
class IndePosition : public LockableControl, public IAdjustTangents {
   public:
	  ...
	  void GetKeyInfo(int i, IKey *key); // only returns time & flags
...
bool IndePosition::GetTangents(TimeValue t, int element, DWORD hitFlags, ParamDimensionBase *dim,float *angle,float *length, DWORD *flags, DWORD *kflags)
{
	...
	switch (element) {
		...
		case 0: {
			IAdjustTangents *adjTanX = GetTangentInterface(posX);
			if (adjTanX)
		...
		case 1: {
			IAdjustTangents *adjTanY = GetTangentInterface(posY);
			if (adjTanY)
		...
		case 2: {
			IAdjustTangents *adjTanZ = GetTangentInterface(posZ);
			if (adjTanZ)
...
void IndePosition::GetKeyInfoTime(TimeValue t, IKey *key)
   ...
   key->flags = -1;
   if (posX) {
	   IKeyControl *ikCtrl = (IKeyControl *)posX->GetInterface(I_KEYCONTROL);
	   if (ikCtrl) {
		   int xIdx = posX->GetKeyIndex(t);
		   if (xIdx >= 0) {
   ...
   if (posY) {
	   IKeyControl *ikCtrl = (IKeyControl *)posY->GetInterface(I_KEYCONTROL);
	   if (ikCtrl) {
		   int yIdx = posY->GetKeyIndex(t);
		   if (yIdx >= 0) {
			   IKey *k = (IKey *)malloc(ikCtrl->GetKeySize());
			   ikCtrl->GetKey(yIdx, k);
			   if (GetInTanType(k->flags)!=BEZKEY_USER)
				   inTanUser = false;
			   if (GetOutTanType(k->flags)!=BEZKEY_USER)
				   outTanUser = false;
   ...
   if (posZ) {
	   IKeyControl *ikCtrl = (IKeyControl *)posZ->GetInterface(I_KEYCONTROL);
	   if (ikCtrl) {
		   int zIdx = posZ->GetKeyIndex(t);
		   if (zIdx >= 0) {
			   IKey *k = (IKey *)malloc(ikCtrl->GetKeySize());
			   ikCtrl->GetKey(zIdx, k);
			   if (GetInTanType(k->flags)!=BEZKEY_USER)
				   inTanUser = false;
			   if (GetOutTanType(k->flags)!=BEZKEY_USER)
				   outTanUser = false;
...
void IndePosition::GetKeyInfo(int i, IKey *key)
   {
   int nk = NumKeys();
