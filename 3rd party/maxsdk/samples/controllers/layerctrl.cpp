...
class LayerControlDlg : public ReferenceMaker, public TimeChangeCallback {
	public:
		...
		virtual void GetClassName(MSTR& s) { s = _M("LayerControlDlg"); } // from Animatable
...
public:
	...
	// this gets executed before some other PLCBs that were deleting the MasterLayerControl
...
RefResult LayerControl::NotifyRefChanged( const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate )
{
	...
	if(needToSetUpParentNode)
	{
		...
		for(int z = 0;z<dep.nodes.Count();++z)
		{
			...
			for(int i=0;i<mLCs.Count();++i)
			{
				...
				MasterLayerControl *mLC = GetMLCMan()->GetNthMLC(mLCs[i]);
				if (mLC)  //post save/load callback scripts can muck with the layers so it may not exist and needs to be checked
					mLC->AddNode(node);
			}
		}
		...
		TimeValue t, void *val, int commit, GetSetMethod method)
{
	...
	MasterLayerControl* pMasterLayerControl = GetMLCMan()->GetNthMLC(mLCs[active]);
	...
	if (method==CTRL_ABSOLUTE)
	{
		...
		weight = pMasterLayerControl?pMasterLayerControl->GetWeight(t,valid):1.0f;
	...
	{
		assert(active>=0);
		if(pMasterLayerControl)
		{
			weight = pMasterLayerControl->GetWeight(t,valid);
			*((float*)val) *= weight;
		}
...
RefResult MasterLayerControl::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) 
{
	...
	//to propagate messages to dependents so such things as redrawing of the trackbar happens correctly
			//Defect #   MZ 797495
			//note that we modified this some more for  868689.  We would propagate REFMSG_CHANGE instead of just the message for any message we received which was wrong.
			//to fix 797495 it looks like we just needed to do this propagation for just these messages which are used by trackbar.
	...
	{
		// null out the reference pointer when the reference target is deleted.
		if (hTarget == pblock)
		{
			pblock = nullptr;
		}
...
int MasterLayerControl::NumRefs() 
{
...
BOOL MasterLayerControl::IsRealDependency(ReferenceTarget *rtarg)
{
	...
	// This solves the issue when loading xref scenes, some temp animatable objects contain animation layers but later it doesn't 
	// get merged but deleted, since previously we hold those layers as strong reference, those layer won't get deleted properly.
	int layerCount = layers.Count();
	for(int i = 0; i < layerCount; ++i)
	{
		if (layers[i] == rtarg)
...
void MasterLayerControl::AddLayerControl(LayerControl* lC)
{
	int j;
...
void MasterLayerControl::GetClassName(TSTR& s)
{
	s = MASTERLAYER_CNAME;
}
...
bool MasterLayerControl::GetLayerInternalFrozen()
{
	int myIndex = GetMLCMan()->GetMLCIndex(this);
	for(int i=0;i<layers.Count();++i)
	{
		...
		if(lC)
		{
			...
			{
				if(lC->mLCs[j]==myIndex &&
					j < lC->mbInternalFrozenFlags.Count())
				{
...
void MasterLayerControl::SetLayerInternalFrozen(bool frozen)
{
	int myIndex = GetMLCMan()->GetMLCIndex(this);
	for(int i=0;i<layers.Count();++i)
	{
...
MaxIcon* MasterLayerControlManagerActionTable::GetIcon(int cmdId)
{
	...
	{
		case ID_ENABLE_ANIM_LAYERS:
			 if (!mi1) mi1 = new MaxBmpFileIcon(_T("Common/Layers")); return mi1; 
		case ID_ANIM_LAYER_PROPERTIES:
			 if (!mi2) mi2 = new MaxBmpFileIcon(_T("AnimationLayer/AnimLayerProperties")); return mi2; 
		case ID_ADD_ANIM_LAYER: 
			 if (!mi3) mi3 = new MaxBmpFileIcon(_T("AnimationLayer/AddAnimLayer")); return mi3; 
		case ID_DELETE_ANIM_LAYER:
			 if (!mi4) mi4 = new MaxBmpFileIcon(_T("AnimationLayer/DeleteAnimLayer")); return mi4; 
		case ID_COPY_ANIM_LAYER:
			 if (!mi5) mi5 = new MaxBmpFileIcon(_T("AnimationLayer/CopyAnimLayer")); return mi5; 
		case ID_PASTE_ACTIVE_ANIM_LAYER:
			 if (!mi6) mi6 = new MaxBmpFileIcon(_T("AnimationLayer/PasteActiveAnimLayer")); return mi6; 
		case ID_PASTE_NEW_LAYER: 
			 if (!mi7) mi7 = new MaxBmpFileIcon(_T("AnimationLayer/PasteNewLayer")); return mi7; 
		case ID_COLLAPSE_ANIM_LAYER: 
			 if (!mi8) mi8 = new MaxBmpFileIcon(_T("AnimationLayer/CollapseAnimLayer")); return mi8; 
		case ID_DISABLE_ANIM_LAYER: 
			 if (!mi9) mi9 = new MaxBmpFileIcon(_T("AnimationLayer/DisableAnimLayer")); return mi9; 
		case ID_SELECT_NODES_FROM_ACTIVE: 
			 if (!mi10) mi10 = new MaxBmpFileIcon(_T("AnimationLayer/SelectActiveLayerObjects")); return mi10; 
...
const TCHAR *MasterLayerControlManager::GetComboToolTip()
{
...
RefResult MasterLayerControlManager::NotifyRefChanged(const Interval& changeInt, RefTargetHandle hTarget, PartID& partID, RefMessage message, BOOL propagate) 
{
	switch (message) 
	{
	case REFMSG_WANT_SHOWPARAMLEVEL:// show the paramblock2 entry in trackview
			...
			if(testMLC!=mLC)
			{
				...
				for(int k=0;k<mLC->layers.Count();++k)
				{
					...
					if(lC)
					{
						...
						// ZCQ: some times it won't find the node for example if the layer control is used as a Custom Attributes' controller
						// then when enumerating its references, you won't find any node reference
						// because CustomAttributeContainer is not hold by any reference maker but as a property of 
						// animatable base object...
						// so we try to add the layerControl no matter a node reference maker is found
						// and we will also make sure the valid nodes monitored by mLC be added to testMLC in the codes below.
						// append node from INodeMonitor. It is safe these nodes&controls are monitored by reference system
						// we don't need worry adding invalid ones or they became invalid later.
						GetNodesProc dep;
				...
				for (int k = 0; k < nodeCount; ++k)
				{
					...
					{
						INodeMonitor *nm = static_cast<INodeMonitor *>(mLC->monitorNodes[k]->GetInterface(IID_NODEMONITOR));
...
void MasterLayerControlManagerPostLoadCallback::proc(ILoad *iload)
{
	...
	MSTR baseLayerName = GetString(IDS_BASE_LAYER);
	for(int j =s->GetNumMLCs()-1;j>-1;--j)
	{
		...
		if(mLC)
		{
			...
			if(mLC->layers.Count()==0
			  && mLC->GetName() != baseLayerName) //should be deleted
			{
				s->DeleteNthMLC(j);
				s->	ResetLayerMLCs(j);
...
void MasterLayerControlManager::GetLayerControlsInternal(Tab<INode*> &nodes, Tab<LCInfo> &existingLayers)
{
...
void MasterLayerControlManager::MakeLayerControl(TrackViewPick res,LayerControl *existingLayer,INode *node)
{
	...
	//now we need to sync up the layer and it's masters, if they exist, otherwise,make sure the layer has the same number of controls.
	int active =0;
...
bool MasterLayerControlManager::GetLayerInternalFrozen(int index)
{
	if(index >= 0 &&
		index < GetNumMLCs())
	{
...
void MasterLayerControlManager::SetLayerInternalFrozen(int index, bool frozen)
{
	if(index >= 0 &&
		index < GetNumMLCs())
	{
...
void IAnimLayerControlManager_Imp::AddLayerForNode(MSTR &name,  ILayerControl* pSourceLayerCtrl,INode* pNode, bool useActiveControllerType)
{
	MasterLayerControlManager *man = GetMLCMan();
...
void MasterLayerControlManager::AddLayerForNode(MSTR &name, ILayerControl* pSourceLayerCtrl,INode* pNode, bool useActiveControllerType)
{
	Tab<MasterLayerControlManager::LCInfo> existingLayers;
	MasterLayerControlManager* man = GetMLCMan();
	MasterLayerControl *mLC = NULL;
...
void AnimLayerToolbarControl::SetSelected(int which)
{
	...
	if (which >= numLayers) which = numLayers - 1;
	if (which < 0) which = 0;
...
INT_PTR MasterLayerControlManager::RenameLayer(HWND hWnd, int which)
{
	...
	if (which==0)
	{
		...
		MessageBox(GetActiveWindow(), GetString(IDS_CANNOT_RENAME_BASE_LAYER), title, MB_OK);
		return 0;
	}
		...
		case IDOK:
			if (which>=0)
			{
				...
				if (len == 0)
				{
					...
					MessageBox(hWnd, msg, GetString(IDS_ANIM_LAYERS), MB_OK);
					return FALSE;
				}
				TSTR newName = bufp;
