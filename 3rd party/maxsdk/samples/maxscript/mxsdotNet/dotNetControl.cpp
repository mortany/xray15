...
dotNetControl::add_control(Rollout *ro, HWND parent, HINSTANCE hInstance, int& current_y)
{
	...
	ScopedValueTempArray evaled_keyparms(keyparm_count*2, _tls);
	...
	m_pDotNetHostWnd->SetWidth(MAXScript::GetUIScaledValue(pos.width));
	m_pDotNetHostWnd->SetHeight(MAXScript::GetUIScaledValue(pos.height));
}

void MXS_dotNet::dotNetControl::adjust_control(int& current_y)
{
	...
	if (hwnd)
	{
		...
		SetWindowPos(hwnd, NULL, MAXScript::GetUIScaledValue(pos.left), MAXScript::GetUIScaledValue(pos.top), MAXScript::GetUIScaledValue(pos.width), MAXScript::GetUIScaledValue(pos.height), SWP_NOZORDER);
	}
...
dotNetControl::get_property(Value** arg_list, int count)
{
	...
	else if(prop == n_height)
	{
		int height = 0;
		if (parent_rollout && parent_rollout->page && m_pDotNetHostWnd)
			height = MAXScript::GetValueUIUnscaled(m_pDotNetHostWnd->GetHeight());
		return_value (Integer::intern(height));
	}
	else if(prop == n_width)
	{
		int width = 0;
		if (parent_rollout && parent_rollout->page && m_pDotNetHostWnd)
			width = MAXScript::GetValueUIUnscaled(m_pDotNetHostWnd->GetWidth());
		return_value (Integer::intern(width));
	}
...
dotNetControl::set_property(Value** arg_list, int count)
{
	...
	{
		if (parent_rollout && parent_rollout->page && m_pDotNetHostWnd)
			m_pDotNetHostWnd->SetHeight(MAXScript::GetUIScaledValue(val->to_int()));
		return val;
	}
	else if (prop == n_width)
	{
		if (parent_rollout && parent_rollout->page && m_pDotNetHostWnd)
			m_pDotNetHostWnd->SetWidth(MAXScript::GetUIScaledValue(val->to_int()));
		return val;
	}
	else if (m_pDotNetHostWnd && (vl.result = m_pDotNetHostWnd->set_property(arg_list, count)) != NULL)
