...
Value* DotNetObjectWrapper::intern(System::Object ^ object)
{
	try
	{
		...
		if (!vl.result)
		{
			if (!ManagedServices::MaxscriptSDK::IsTypeWhitelistedForSecureMode(object->GetType(), true))
			{
				MSTR l_pTypeString = MXS_dotNet::MNETStr::ToMSTR(object->GetType()->FullName);
				throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_INSTANCE_TYPE_UNAVAIL), l_pTypeString);
			}
...
Value* DotNetObjectWrapper::get_property(Value** arg_list, int count)
{
	...
	try
	{
			...
			if (l_pPropertyInfo) 
			{
				...
				if (l_pAccessor) 
				{
					...
					if (l_pParams->Length != 0)
						return_value (DotNetMethod::intern(l_pAccessor->Name, this, l_pType));
		...
		if (!found)
		{
			...
			if (l_pMethodInfo) 
				return_value (DotNetMethod::intern(l_pMethodName, this, l_pType));
		}
		if (!found)
			return NULL;
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	...
	try
	{
		...
		for(int f=0; f<fields->Length; f++)
		{
			...
			if (System::String::Compare(pPropString, pFieldName, StringComparison::OrdinalIgnoreCase) == 0) 
			{
				...
				System::Object ^ pVal = MXS_dotNet::MXS_Value_To_Object(val, pFieldInfo->FieldType, pFieldInfo->Name, backingFPValues);
		...
		if (!found)
		{
			...
			if (pPropertyInfo) 
			{
				...
				if (accessor) 
				{
					...
					if (params->Length != 1)
						return_value (DotNetMethod::intern(accessor->Name, this, type));

					System::Object ^ pVal = MXS_dotNet::MXS_Value_To_Object(val, pPropertyInfo->PropertyType, pPropertyInfo->Name, backingFPValues);
													  ...
													  System::Reflection::MethodInfo ^mi)
{
	...
	try
	{
		...
		// Do not subscribe to any events in State Sets assembly until we figure out what causes 3ds max to hang when doing so
		if( targetType->Assembly->FullName->StartsWith( gcnew System::String( "Autodesk.Max.StateSets" ) ) )
		{
			return;
		}
...
void DotNetObjectWrapper::RunEventHandlers(Value* target, Value* eventName, Array* handlers, array<System::Object^> ^delegateArgsArray)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	value_local_array_tls(arg_list, dArgs);
	...
	for (int i = 0; i < handlers->size; i++)
	{
		...
		if (is_function(handler))
		{
			try 
			{
				...
				int cArgs = 0;
				GetMAXScriptFunctionParameterCount(handler, cArgs);
			...
			{
				bool inTryMode = _tls->try_mode != FALSE;
				ProcessMAXScriptException(e, _M("dotNet event handler"), true, true, !inTryMode);
				if (inTryMode)
				{
					throw;
				}
			...
			{
				bool inTryMode = _tls->try_mode != FALSE;
				ProcessMAXScriptException(UnknownSystemException(), _M("dotNet event handler"), true, true, !inTryMode);
				if (inTryMode)
				{
					throw;
				}
...
void DotNetObjectWrapper::ProcessEvent(System::String ^eventName, System::Object ^delegateArgs)
{
	...
	two_value_locals_tls(eventName, handlers);
	try
	{
