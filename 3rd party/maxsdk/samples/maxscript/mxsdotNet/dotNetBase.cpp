...
dotNetBase::dotNetBase() : m_pEventHandlers(NULL), m_AtomFlag(FALSE)
{
#ifdef _DEBUG
	instanceCounter++;
#endif
...
static bool sSupportSpinLock = false;
...
dotNetBase::add_event_handler(Value* eventName, Value* handler)
{
	...
	if (!wrapper->HasEvent(eventName->to_string()))
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NO_EVENT_FOR_OBJECT), eventName);
	SpinLock(m_AtomFlag);
	if (!m_pEventHandlers)
...
dotNetBase::remove_event_handler(Value* eventName, Value* handler)
{
	...
	if (m_pEventHandlers)
	{
		...
		if (handlerFns)
		{
			...
			{
				Value* handlerFn = (*handlerFns)[i];
				if ((handler == handlerFn) || IsSameMAXScriptFunction(handler, handlerFn, _tls))
				{
