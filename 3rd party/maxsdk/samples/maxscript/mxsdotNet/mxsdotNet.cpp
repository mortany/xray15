...
// following is to prevent 'warning LNK4248: unresolved typeref token ' - we compare the address of 
// an instance of this class, but we don't need to know the contents of the class instance.
...
dotNet_add_event_handler_cf(Value** arg_list, int count)
{
	...
	Value* val = arg_list[0];
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_value_locals_tls(fn,eventName);
	const MCHAR *eventName = arg_list[1]->to_string();
	vl.eventName = Name::intern(eventName);
	vl.fn = arg_list[2];
	if (!is_function(vl.fn))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_BAD_EVENT_HANDLER_TYPE), vl.fn);
	vl.fn = CreateWrappedMAXScriptFunction(vl.fn, _tls);
...
dotNet_set_lifetime_control_cf(Value** arg_list, int count)
{
	...
	if (arg_list[1] == n_mxs)
		...
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_BAD_LIFETIME_ARG), arg_list[1]);
...
{
	// loadAssembly <assembly_dll_string> returnPassFail:<bool>
	check_arg_count_with_keys(dotNet.loadAssembly, 1, count);
	MSTR  fileName = arg_list[0]->to_filename();
	Value* dmy;
	bool returnPassFail = bool_key_arg(returnPassFail, dmy, FALSE) != FALSE;
	AssetId assetId;
	...
	return LoadAssembly(fileName, returnPassFail);
}
...
dotNet_Value_To_DotNetObject_cf(Value** arg_list, int count)
{
	...
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(MXSValueToDotNetObjectValue(arg_list[0], type));
	}
	else
...
Value*
dotNet_DotNetArray_To_Value_cf(Value** arg_list, int count)
{
	// <value> DotNetArrayToValue <dotNetObject> <Matrix3 | Point2 | Point3 | Point4 | Quat | EulerAngles>
	check_arg_count(dotNet.DotNetArrayToValue, 2, count);
	dotNetBase *obj = (dotNetBase*)arg_list[0];
	ValueMetaClass* type = (ValueMetaClass*)arg_list[1];
	if (!is_valueclass(type))
		throw ConversionError(type, _M("Value Class"));
	if (is_dotNetObject(obj))
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(DotNetArrayToMXSValue(obj, type));
	}
	else
		throw ConversionError(obj, _M("dotNetObject"));
}

Value*
dotNet_DotNetObject_To_Value_cf(Value** arg_list, int count)
{
	// <value*> DotNetObjectToValue <dotNetObject>
	check_arg_count(dotNet.DotNetObjectToValue, 1, count);
	dotNetBase *obj = (dotNetBase*)arg_list[0];
	if (is_dotNetObject(obj))
	{
		MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
		return_value_tls(DotNetObjectValueToMXSValue(obj));
	}
	else
		throw ConversionError(obj, _M("dotNetObject"));
}
...
Value*
dotNet_loadIcon_cf(Value** arg_list, int count)
{
	// dotNet.LoadIcon <iconName> iconSize:<point2> enabled:<bool> on:<bool> applyUIScaling:<bool>-- defaults values: [24,24], true, false, true
	check_arg_count_with_keys(dotNet.loadIcon, 1, count);
	TSTR iconName = arg_list[0]->to_filename();
	Value* iconSize = key_arg(iconSize);
	int image_width, image_height;
	if (iconSize != &unsupplied)
	{
		Point2 p = iconSize->to_point2();
		image_width = (int)p.x;
		image_height = (int)p.y;
	}
	else
		image_width = image_height = 24;
	Value* v;
	bool enabled = bool_key_arg(enabled, v, TRUE) != FALSE;
	bool on = bool_key_arg(on, v, FALSE) != FALSE;
	BOOL applyUIScaling = bool_key_arg(applyUIScaling, v, TRUE);
	if (applyUIScaling)
	{
		image_height = MAXScript::GetUIScaledValue(image_height);
		image_width = MAXScript::GetUIScaledValue(image_width);
	}
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	return_value_tls(CreateIconFromMaxMultiResIcon( iconName, image_width, image_height, enabled, on ));
}
