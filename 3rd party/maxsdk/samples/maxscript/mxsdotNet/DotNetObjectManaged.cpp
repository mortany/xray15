	...
	try
	{
		...
		if (l_pType)
		{
			...
			if (count != 0)
			{
				...
				{
					System::Object ^l_pObject = MXS_dotNet::MXS_Value_To_Object(evald_args[0],l_pType, nullptr, backingFPValues);
				...
				int bestMatch = MXS_dotNet::FindMethodAndPrepArgList(candidateParamLists, evald_args, 
					argArray, arg_count, userSpecifiedParamTypeArray, backingFPValues);
				if (bestMatch >= 0)
			...
			if (l_pConstructorInfo)
				...
				throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_NO_MATCHING_CTOR), pTypeString);
			if (l_pObject)
			{
				...
				throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_OBJECT_CTOR_FAILED), pTypeString);
		}
		else
			throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_RESOLVE_TYPE), pTypeString);
	}
...
void DotNetObjectManaged::delegate_proxy_type::detach()
{
	...
	try
	{
		...
		if (target)
		{
			...
			if (m_pEventDelegatePairs)
			{
				// there is a bug associated with frozen objects in .NET 4.0 and earlier. It is throwing a 
				// InvalidOperationException when removing event handlers. This bug is supposively fixed in .NET 4.5.
				// see https://connect.microsoft.com/VisualStudio/feedback/details/650350/wpf-4-0-freezable-bug-writepreamble-exception-while-loading-a-ui-element
				System::Windows::Freezable ^ freezableTarget = dynamic_cast<System::Windows::Freezable^>(target);
				bool frozenObject =  (freezableTarget && freezableTarget->IsFrozen);
				static bool isv40 = Assembly::GetExecutingAssembly()->ImageRuntimeVersion->StartsWith("v4.0");
				DbgAssert (isv40);
				if (!frozenObject || !isv40)
				{
...
Value* DotNetObjectManaged::Get(Value* index)
{
	System::Type ^ l_pType = GetType();
	Value* args[1] = { index };
	return_value(DotNetMethod("Get", l_pType).apply(this, args, 1, false));
}

void DotNetObjectManaged::Set(Value* index, Value* val)
{
	System::Type ^ l_pType = GetType();
	Value* args[2] = { index, val };
	DotNetMethod("Set", l_pType).apply(this, args, 2, false);
}
