...
dotNetMXSValueClass::apply(Value** arg_list, int count, CallContext* cc)
{
	...
	return_value (dotNetMXSValue::intern(arg_list[0]->eval()));
}
...
dotNetMXSValue::intern(Value *pMXSValue)
{
	...
	one_typed_value_local_tls(dotNetMXSValue* result);
	if (!m_pExisting_dotNetMXSValues)
...
dotNetMXSValue::set_property(Value** arg_list, int count)
{
	...
	if (prop == n_value)
	{
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_READ_ONLY_PROPERTY), _T("value"));
	}
	else
