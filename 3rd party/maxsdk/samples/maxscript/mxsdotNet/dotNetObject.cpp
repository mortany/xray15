...
dotNetObjectClass::apply(Value** arg_list, int count, CallContext* cc)
{
	...
	two_value_locals_tls(arg, result);
...
Value*
dotNetObject::get_vf(Value** arg_list, int count)
{
	check_arg_count(get, 2, count + 1);
	return m_pDotNetObject->Get(arg_list[0]);
}

Value*
dotNetObject::put_vf(Value** arg_list, int count)
{
	check_arg_count(put, 3, count + 1);
	m_pDotNetObject->Set(arg_list[0], arg_list[1]);
	return arg_list[1];
}
