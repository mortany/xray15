...
CreatePreview_cf(Value** arg_list, int count)
{
	...
	MAXScript_interface->CreatePreview(&previewParams, &mFilename, &mSnippet);
	return &ok;
}
...
loadDllsFromDir_cf(Value** arg_list, int count)
{
	...
	if (GetCOREInterface17()->InSecureMode())
		throw RuntimeError(MaxSDK::GetResourceStringAsMSTR(IDS_UNAVAIL_IN_SECURE_MODE), _T("LoadDllsFromDir"));
...
GetMasterController_cf(Value** arg_list, int count)
{
	...
	if (tmControl) {
		return_value (MAXRefTarg::intern(static_cast<ReferenceTarget*>(tmControl->GetInterface(I_MASTER))));
	}
	return &undefined;
}
...
fp_get_property_val_cf(Value** arg_list, int count)
{
	...
	switch(pProp->vt) {
		...
		case VT_I1:		return_value (Integer::intern(pProp->cVal));
		case VT_UI1:	return_value (Integer::intern(pProp->bVal));
		case VT_I2:		return_value (Integer::intern(pProp->iVal));
		case VT_UI2:	return_value (Integer::intern(pProp->uiVal));
		case VT_I4:		return_value (Integer::intern(pProp->lVal));
		case VT_UI4:	return_value (Integer::intern(pProp->ulVal));
		case VT_I8:		return_value (Integer64::intern(pProp->hVal.QuadPart));
		case VT_UI8:	return_value (Integer64::intern(pProp->uhVal.QuadPart));
		case VT_INT:	return_value (Integer::intern(pProp->intVal));
		case VT_UINT:	return_value (Integer::intern(pProp->uintVal));
		case VT_INT_PTR:
#if !defined( _WIN64 )
...
#endif
		case VT_R4:		return_value (Float::intern(pProp->fltVal));
		case VT_R8:		return_value (Double::intern(pProp->dblVal));			
...
SetRenderType_cf(Value **arg_list, int count)
{
	...
	int rtype = ConvertValueToID(renderTypes, elements(renderTypes), arg_list[0]);
