...
class TrackMouseCallBack : public MouseCallBack, ReferenceMaker
{
	public:
		...
		virtual void GetClassName(MSTR& s) { s = _M("TrackMouseCallBack"); } // from Animatable
		// ReferenceMaker
...
do_track_callback(TrackMouseCallBack* ppcb, Value* message, int flags)
{
	...
	catch (...)
	{
		...
		ppcb->track_callback = NULL;
		ppcb->vl->result = &undefined;
		ProcessMAXScriptException(UnknownSystemException(), _T("TrackMouseCallBack function"), false, true, true);
	}
