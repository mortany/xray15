...
{
	if (to->isShallowCopy) {
		DbgAssert(to->tmControl == nullptr && to->pblock == nullptr && to->masterCont == nullptr);
		to->tmControl = tmControl;
		to->pblock = pblock;
		to->masterCont = masterCont;
		to->ptCont = ptCont;
	}
	else {
		if (tmControl) to->ReplaceReference(0, remap.CloneRef(tmControl));
		to->ReplaceReference(1, remap.CloneRef(pblock));
		// NOTE: MasterPointControlImp::Clone does not clone the controllers it holds. No idea why not.... 
		to->ReplaceReference(2, remap.CloneRef(masterCont));
		int numPts = NumPts();
		...
		for (int i = 0; i < numPts; i++) {
			...
			if (pCont)
				to->ReplaceReference(i + 3, remap.CloneRef(pCont)); // this sets element in ptCont and masterCont
		}
	}
	to->pt = pt;
	to->offsets = offsets;
	...
	for (int ix = 0; ix < 4; ix++) {
		for (int jx = 0; jx < 4; jx++) {
			...
			for (int kx = 0; kx < 4; kx++) {
				...
				DbgAssert( 0 <= ii && ii < (dim[0] + (ffd->WrapX() ? 0 : 2)) );
				if ( !(0 <= ii && ii < (dim[0] + (ffd->WrapX() ? 0 : 2))) ) {
