...
//					vecBtoA = pa-pb;
	...
	int numThreads = threadData.GetNumberOfThreads(MaxSDK::PerformanceTools::ThreadTools::kDeformationThreading,mesh->numFaces);
...
/*
	...
	*/
	...
	for (int j = 0; j < mesh->numFaces; j++)
		...
		vnorms[ia] += norm * cornerAngles[j].a * (1.0f /(PI*2));
		SpinUnlock(vFlagPtr[ia]);

		SpinLock(vFlagPtr[ib]);
		vnorms[ib] += norm * cornerAngles[j].b * (1.0f /(PI*2));
		SpinUnlock(vFlagPtr[ib]);

		SpinLock(vFlagPtr[ic]);
		vnorms[ic] += norm * cornerAngles[j].c * (1.0f /(PI*2));
