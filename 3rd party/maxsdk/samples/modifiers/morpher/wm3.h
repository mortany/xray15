...
#define MR3_MC_SUBCHUNK		0x0200    ///NOTE NO LOAD CHUNKS ID MUST BE PAST THIS SINCE THIS IS THE START OF THE ARRAY OF CHANNELS WHICH IS UNDEFINED
...
public:
	...
	// Most channels will start out by using this function.
        // inModify means that the we are inside an evaluation which limits what we should do like update tghe UI and send notify messages
	MorphExport void buildFromNode( INode *node , BOOL resetTime=TRUE , TimeValue t=0, BOOL picked = FALSE, BOOL inModify = FALSE );
...
public:
		...
		int GetRefNumber(int chanNum, int targNum) { return (200 + targNum + ((chanNum%100) * MAX_PROGRESSIVE_TARGETS)); }
...
class M3Mat : public Mtl, public IReshading  {	
	public:
		...
		int NumSubMtls() {return mTex.Count();}
		Mtl* GetSubMtl(int i) {
			if (i < mTex.Count())
				return mTex[i];
			return nullptr;
		...
		TSTR GetSubMtlSlotName(int i) {
			...
			if(morphp) 
			{
				...
				{
					_stprintf(s,_T("Mtl %i (%s)"),i+1,morphp->chanBank[i].mName.data());
					return s;
				}
			...
			else 
			{
				if ( i > 100)
					i--;
				_stprintf(s,GetString(IDS_MTL_MAPNAME),i+1);
				return s;
			}
		...
		int NumSubs() {return mTex.Count()+1;} 
		Animatable* SubAnim(int i);
		TSTR SubAnimName(int i);
		int SubNumToRefNum(int subNum) {return subNum;}

		// From ref
 		int NumRefs() {return mTex.Count()+3;}
		RefTargetHandle GetReference(int i);
private:
		virtual void SetReference(int i, RefTargetHandle rtarg);
public:
...
class MorphR3PostLoadCallback : public PostLoadCallback {
	public:
		MorphR3 *mp;
		MorphR3PostLoadCallback() {mp=NULL;}
