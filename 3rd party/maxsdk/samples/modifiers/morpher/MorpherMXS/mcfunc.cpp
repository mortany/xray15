...
wm3_mc_getname_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_GetName [Morpher Modifier] [Channel Index]"));
...
wm3_mc_setname_cf(Value** arg_list, int count)
{
	...
	integer_type_check(arg_list[1], _T("WM3_MC_SetName [Morpher Modifier] [Channel Index] [Name String]"));
...
wm3_mc_setactive_cf(Value** arg_list, int count)
{
	...
	if (!is_integer_number(arg_list[2]))
...
wm3_mc_getnummpts_cf(Value** arg_list, int count)
{
	...
	return_value (Integer::intern(static_cast<int>(mp->chanBank[sel].mPoints.size())));	// SR DCAST64: Downcast to 2G limit.
}
