					...
					else
					{
						...
						if (boxCount)
						{
							...
							for (int k = 0; k < boxCount;  k++)
							{
								...
								min = boxPtr[k].min;
								max = boxPtr[k].max;
								if ( (p2.x <= boxPtr[k].max.x) && (p2.x >= boxPtr[k].min.x) &&
									 (p2.y <= boxPtr[k].max.y) && (p2.y >= boxPtr[k].min.y) )
								{
									Point3 plist[3];
									int index = 0;
									for (int f = 0; f < 3; f++)
		...
		// be in the range 0-1
				...
				if ((useAR) &&( msh != NULL))
				{
					...
					if (bboxFalloff.Contains(pt))
					{
						float closest = -1.0f;
						Box3* box3DPtr = NULL;
						int boxCount = box3DList.Count();
						if (boxCount > 0)
						{
							box3DPtr = box3DList.Addr(0);
						}
						for (int k = 0; k < boxCount;  k++)
						{
							if (box3DPtr[k].Contains(pt))
							{
...
//	#pragma omp parallel for 
	for (int i=0; i<mesh.getNumVerts(); i++) {
		...
		if ((tvFace!= NULL) &&(i<uvwList.Count())) tv = uvwPtr[i];
		fTable[i] = PointInVolume(t, mesh.verts[i],tv.x,tv.y,tv.z,tm,box,i);
	}
	...
	for (int i=0; i<mesh.getNumVerts(); i++) {
		...
		if (f == 1.0f) {
			if (method==SEL_SUBTRACT) {
				mesh.vertSel.Clear(i);     // not thread safe
				if (vsw) vsw[i] = 0.0f;
			} else {
				mesh.vertSel.Set(i);      // not thread safe
				if (vsw) vsw[i] = 1.0f;
			}
		} else {
			if (vsw) {
...
//	int numThreads = omp_get_num_procs();
//	omp_set_num_threads(numThreads);
//	#pragma omp parallel for schedule(static, mesh.numv / numThreads)
	for (int i=0; i<mesh.numv; i++) {
		//get texture verts for that vertex
		//loop through faces find ing matched
		Point3 tv(0.5f,0.5f,0.0f);
		if (texmap &&(i<uvwList.Count())) tv = uvwPtr[i];
...
//	#pragma omp parallel for schedule(dynamic, 1024)
	for (int i=0; i<pmesh.getNumVerts(); i++) {
		...
		if ((tvPatch!= NULL) &&(i<uvwList.Count()))
			tv = uvwList[i];
		fTable[i] = PointInVolume(t, pmesh.verts[i].p,tv.x,tv.y,tv.z,tm,box,i);
	}
	for (int i=0; i<pmesh.getNumVerts(); i++) {
		float f = fTable[i];
		if (f == 1.0f) {
			if (method==SEL_SUBTRACT) {
				pmesh.vertSel.Clear(i);       // not thread safe
				if (vsw)
					vsw[i] = 0.0f;
			} else {
				pmesh.vertSel.Set(i);		     // not thread safe
				if (vsw)
					vsw[i] = 1.0f;
			}
		} else {
...
void SelMod::SelectFaces (TimeValue t, Mesh &mesh, Matrix3 &tm, Box3 &box) {
	...
	Tab<float> fTable;
	fTable.SetCount(mesh.getNumFaces()*3);
	int numThreads = omp_get_num_procs();
	omp_set_num_threads(numThreads);
//	#pragma omp parallel for schedule(dynamic, 1024)
	for (int i=0; i<mesh.getNumFaces(); i++) {
		for (int k=0; k<3; k++) {	
			...
			fTable[i*3+k] = PointInVolume(t,mesh.verts[mesh.faces[i].v[k]],tv.x,tv.y,tv.z,tm,box,mesh.faces[i].v[k]);
		}
	}
...
void SelMod::SelectFaces (TimeValue t, MNMesh &mesh, Matrix3 &tm, Box3 &box) {
	...
	Tab<int> fTableIndex;
	fTableIndex.Resize(mesh.numf);
	int runningIndex = 0;
	for (int i=0; i<mesh.numf; i++) {
		if (mesh.f[i].GetFlag(MN_DEAD))
		{
			fTableIndex.Append(1, &runningIndex, 10000); //still need to insert a blank entry if a dead face is encountered
		}
		else
		{
			fTableIndex.Append(1, &runningIndex, 10000);
			runningIndex += mesh.f[i].deg;
		}
	}
	Tab<float> fTable;
	fTable.SetCount(runningIndex);


//	int numThreads = omp_get_num_procs();
//	omp_set_num_threads(numThreads);
//	#pragma omp parallel for schedule(static, mesh.numf / numThreads)
	for (int i=0; i<mesh.numf; i++) {
		...
		for (int k=0; k<mesh.f[i].deg; k++) {
			...
			int index = fTableIndex[i] + k;
			fTable[index] = PointInVolume(t, mesh.v[mesh.f[i].vtx[k]].p,tv.x,tv.y,tv.z,tm,box,mesh.f[i].vtx[k]);
		}
	}
	...
	for (int i=0; i<mesh.numf; i++) {
		...
		for (int k=0; k<mesh.f[i].deg; k++) {
			...
			if (fTable[fTableIndex[i]+k]==1.0f) {
				in++;
				if (selType==SEL_CROSSING) break;
			} else {
