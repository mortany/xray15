			...
			for (int k = 0; k < ld->GetNumberFaces(); k++)
			{
				if (ld->GetFaceSelected(k))
				{
					...
					for (int j = 0; j<pcount; j++)
					{
						...
						bounds += ld->GetGeomVert(index) * tm;
					}
				}
			}
		}
...
void UnwrapMod::fnAlignAndFit(int axis)
{
	...
	else if (axis == 3) // normal
	{
		...
		if ((fnGetMapMode() == PLANARMAP) || (fnGetMapMode() == PELTMAP) || (numberOfSelectionGroups > 1))
		{
			//get our tm
			Matrix3 tm;
			UnwrapUtilityMethods::UnwrapMatrixFromNormal(pnorm,tm);
				...
				if (ld->GetFaceSel().AnyBitSet())
				{
					...
					for (int k = 0; k < ld->GetNumberFaces(); k++)
					{
						if (ld->GetFaceSelected(k))
						{
							...
							{
								int index = ld->GetFaceGeomVert(k, j);
								Point3 p = ld->GetGeomVert(index) * tm * itm;
								localBounds += p;
							}
						}
					}
...
//			center = localBounds.Center();
		...
		else if ((fnGetMapMode() == CYLINDRICALMAP) || (fnGetMapMode() == SPHERICALMAP)|| (fnGetMapMode() == BOXMAP))
		{
			...
			for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
			{
					...
					for (int k=0; k < ld->GetNumberFaces(); k++) 
					{
						if (ld->GetFaceSelected(k))
						{
							...
							for (int j=0; j<pcount; j++) 
							{
								...
								Point3 p = ld->GetGeomVert(index)* nodeTM;
				...
				//get our tm
				Matrix3 tm;
				UnwrapUtilityMethods::UnwrapMatrixFromNormal(zVec,tm);
				tm.SetRow(3,centerS);
 				Matrix3 itm = Inverse(tm);
				//find our x and y scale
				float xmax = 0.0f;
				...
				for (int k = 0; k < ld->GetNumberFaces(); k++)
				{
					if (ld->GetFaceSelected(k))
					{
						...
						{
							int index = ld->GetFaceGeomVert(k,j);
							Point3 p = ld->GetGeomVert(index) * nodeTM * itm;
							localBounds += p;
						}
					}
				}
...
void UnwrapMod::fnGizmoFit()
{
		...
		if (ld->GetFaceSel().AnyBitSet())
		{
			...
			for (int k = 0; k < ld->GetNumberFaces(); k++)
			{
				if (ld->GetFaceSelected(k))
				{
					...
					{
						int index = ld->GetFaceGeomVert(k, j);
						bounds += ld->GetGeomVert(index) * wtm;
					}
					ct++;
				}
			}
		...
		if (ld->GetFaceSel().AnyBitSet())
		{
			...
			for (int k = 0; k < ld->GetNumberFaces(); k++)
			{
				if (ld->GetFaceSelected(k))
				{
					...
					{
						int index = ld->GetFaceGeomVert(k, j);
						Point3 p = ld->GetGeomVert(index)* wtm * itm;
						localBounds += p;
					}
				}
			}
	...
	ymax = localBounds.pmax.y - localBounds.pmin.y;
	zmax = localBounds.pmax.z - localBounds.pmin.z;
	//keep the same logic with the one in the function fnAlignAndFit() 
	if (xmax == 0.0f) xmax = 1.0f;
	if (ymax == 0.0f) ymax = 1.0f;
	if (zmax == 0.0f) zmax = 1.0f;
		...
		for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
		{
			...
			for (int k = 0; k < 6; k++)
			{
				...
				if (k == 0)
				{
					...
					tm.SetRow(1,zvec);
					tm.SetRow(2,xvec);
					ld->SetFaceSelectionByRef(front);
				}
				else if (k == 1)
				{
					tm.SetRow(0,yvec);
				...
				else if (k == 3)
				{
					...
					ld->SetFaceSelectionByRef(right);
				}
				else if (k == 4)
				{
					tm.SetRow(0,xvec);
				...
				else if (k == 5)
				{
					...
					ld->SetFaceSelectionByRef(bottom);
				}
				...
				ld->ApplyMap(fnGetMapMode(), fnGetNormalizeMap(), tm, GetFilterMatID());
			}
			ld->RestoreFaceSel();
		}
