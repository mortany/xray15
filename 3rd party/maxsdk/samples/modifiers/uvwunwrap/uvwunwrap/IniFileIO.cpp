...
bool IniFileIO::LoadIniFile(const TSTR& strFileName)
{
	...
	FILE* pFile;
	_wfopen_s(&pFile,strFileName.data(), _T("rt, ccs=UTF-16LE"));
	if(NULL == pFile)
	{
		return false;
	}
	...
	TSTR strKey;
	TSTR strValue;
	auto res = _fgetts(strReadLineBuf, MAX_LINE_BUF_SIZE, pFile);
	if (NULL == res)
	{
		fclose(pFile);
		return false;
	...
	do
	{
		...
		{
			//if its not a whole line, appened onto a running buffer until it is.
		...
		res = _fgetts(strReadLineBuf, MAX_LINE_BUF_SIZE, pFile);
	}
	while (NULL != res);
...
bool IniFileIO::SaveAs(const TSTR& strFileName)
{
	...
	FILE *pFile;
	_wfopen_s(&pFile,strFileName.data(), _T("wt, ccs=UTF-16LE"));
	if(NULL == pFile)
	{
		return false;
	}
