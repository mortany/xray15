...
unsigned __stdcall  CallPeltThread(void *data)
{
	...
	theHold.Put(new UnwrapPeltVertVelocityRestore(threadData->mod->peltData));
	...
	HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		UnwrapMod::hMapParams = hWnd;
		mod->peltData.ShowUI(hWnd, FALSE);

		mod->EnableMapButtons(FALSE);
		mod->EnableAlignButtons(FALSE);

		mod->peltData.EnablePeltButtons(mod, UnwrapMod::hMapParams, FALSE);
...
void PeltData::PeltMode(UnwrapMod *mod, BOOL start)
{
	...
	ShowUI(hModMapParams, inPeltMode);
	mod->GetUIManager()->UpdateCheckButtons();
...
void PeltData::SetPointToPointSeamsMode(UnwrapMod *mod, BOOL mode, BOOL selSeams)
{
	...
	currentGeomHit.Reset();
	previousGeomHit.Reset();
...
void PeltData::SnapRig()
{
	...
	for (int i = 0; i < rigPoints.Count(); i++)
	{
		...
		{
			Point3 targetP = ld->GetTVVert(targetIndex);
			ld->SetTVVert(t, selfIndex, targetP);	
		}
	}
}
...
void PeltData::CleanUpIsolatedClusters(BOOL byTVs)
{
	//now get our largest cluster and only use that cluster
	theHold.Suspend();
	BitArray holdFaces = mBaseMeshTopoDataCurrent->GetFaceSel();  //original selection

	peltFaces = mBaseMeshTopoDataCurrent->GetFaceSel(); //were we are going to store off our final selection
	peltFaces.ClearAll();
	...
	mBaseMeshTopoDataCurrent->SetFaceSelectionByRef(peltFaces);
	theHold.Resume();
}
...
void PeltData::SetupSprings(UnwrapMod *mod)
{
	...
	if (theHold.Holding())
	{
		theHold.Put(new HidePeltDialogRestore(mod));
	}
	...
	{
		MeshTopoData *ld = mod->GetMeshTopoData(ldID);
		if (ld->GetFaceSel().NumberSet() > faceCount)
		{
			currentLD = ldID;
			mBaseMeshTopoDataCurrent = ld;
			faceCount = ld->GetFaceSel().NumberSet();
		}
	}
	...
	mBaseMeshTopoDataCurrent->SetTVVertSel(vsel);

	mBaseMeshTopoDataCurrent->SetFaceSelectionByRef(peltFaces);//md->faceSel = peltFaces;
	mod->SyncTVFromGeomSelection(mBaseMeshTopoDataCurrent);
	...
	vsel = mBaseMeshTopoDataCurrent->GetTVVertSel();
	hiddenVerts = vsel;
	...
	mBaseMeshTopoDataCurrent->ClearTVVertSelection();

	if (mod->matid != -1) // if we have a matID fileter set we need to rebuild since topology has changed
		mod->SetMatFilters();
}
...
void PeltData::SelectPelt(UnwrapMod *mod, BOOL replace)
{
	...
	BitArray vsel = mBaseMeshTopoDataCurrent->GetTVVertSel();
...
void PeltData::ResetRig(UnwrapMod *mod)
{
	...
	for (int i = 0; i < springEdges.Count(); i++)
	{
		...
		if (index >= 0)
		{
			...
			{
				ld->SetTVVert(t, index, initialPointData[index]);
			}
...
void PeltData::CutSeams(BitArray seams)
{
	mBaseMeshTopoDataCurrent->CutSeams(seams);
}
...
static void ResizeWindows(HWND hWnd, int bottomHeight)
{
	...
	float dpiScale = MaxSDK::GetUIScaleFactor();
	int x = 2 * dpiScale;
	int y = 1 * dpiScale;
	int width = rect.w() - 12 * dpiScale;
	int height = bottomHeight - 40 * dpiScale;
...
void PeltDialog::SetStraightenMode(BOOL on)
{
	...
	else
	{
		...
		mod->peltData.previousGeomHit.Reset();
...
UnwrapPeltSeamRestore::UnwrapPeltSeamRestore(UnwrapMod *m, MeshTopoData *d, Point3 *anchorPoint)
{
	...
	ld = d;
	useams = d->mSeamEdges;
	up1 = mod->peltData.previousGeomHit;
	up2 = mod->peltData.currentGeomHit;
...
void UnwrapPeltSeamRestore::Restore(int isUndo)
{
	if (isUndo)
	{
		...
		rp1 = mod->peltData.previousGeomHit;
		rp2 = mod->peltData.currentGeomHit;
	...
	mod->peltData.previousGeomHit.Reset();
	mod->peltData.currentGeomHit = up1;
	mod->peltData.basep = ulastP;
...
{
	mod->peltData.previousGeomHit.Reset();
	mod->peltData.currentGeomHit = rp1;
...
UnwrapPeltVertVelocityRestore::UnwrapPeltVertVelocityRestore(PeltData& p) : peltData(p)
{
	uverts = peltData.verts;
}
...
void UnwrapPeltVertVelocityRestore::Restore(int isUndo)
{
	if (isUndo)
	{
		rverts = peltData.verts;
	}
	peltData.verts = uverts;
}

void UnwrapPeltVertVelocityRestore::Redo()
{
	peltData.verts = rverts;
}
...
void PeltData::MirrorRig(UnwrapMod *mod)
{
	...
	Point3 p = ld->GetTVVert(sid);

	p = p*itm;
	p.x *= -1.0f;
	ld->SetTVVert(t, id, p*tm);
	...
	while (lastMP.x > 0.0f)
	{
		...
		Point3 prevp = ld->GetTVVert(rigPoints[prevSourcePoint].lookupIndex);
		Point3 currentp = ld->GetTVVert(rigPoints[currentSourcePoint].lookupIndex);
		...
		ld->SetTVVert(t, rigPoints[currentTargetPoint].lookupIndex, p);
	}

	mod->InvalidateView();
}

void PeltData::StraightenSeam(int a, int b)
{
	//get our sel
	BitArray seamPoints;
	MeshTopoData *ld = mBaseMeshTopoDataCurrent;
	seamPoints.SetSize(ld->GetNumberTVVerts());
	seamPoints.ClearAll();
	for (int i = 0; i < rigPoints.Count(); i++)
	{
	...
	pa = ld->GetTVVert(rigPoints[startPoint].lookupIndex);
	pb = ld->GetTVVert(rigPoints[endPoint].lookupIndex);
...
int PeltStraightenMode::subproc(HWND hWnd, int msg, int point, int flags, IPoint2 m)
{
	...
	case MOUSE_POINT:
		if (point == 0) {
			...
			if (mod->peltData.previousGeomHit.IsValid() && mod->peltData.previousGeomHit.mGeomType == HitGeomType_Vertex)
			{

				mod->peltData.StraightenSeam(mod->peltData.previousGeomHit.mIndex, mod->peltData.currentGeomHit.mIndex);
				TSTR mstr = mod->GetMacroStr(_T("modifiers[#unwrap_uvw].unwrap5.peltDialogStraighten"));
				macroRecorder->FunctionCall(mstr, 2, 0,
					mr_int, mod->peltData.previousGeomHit.mIndex + 1,
					mr_int, mod->peltData.currentGeomHit.mIndex + 1
					);
			}
	...
	case MOUSE_MOVE: {
		...
		for (int i = 0; i < ld->GetNumberTVVerts(); i++)//mod->vsel.GetSize(); i++)
		{
			if (ld->GetTVVertSelected(i))//if (mod->vsel[i])
				mod->peltData.currentGeomHit.mIndex = i;
			mod->peltData.currentGeomHit.mGeomType = HitGeomType_Vertex;
		}
...
int PeltData::HitTestPointToPointMode(UnwrapMod *mod, MeshTopoData *ld, ViewExp& vpt, GraphicsWindow *gw, IPoint2 *p, HitRegion hr, INode* inode, ModContext* mc, const BitArray *preCaledVisibleFaces)
{
	if (!vpt.IsAlive())
	{
		// why are we here
	...
	if (preCaledVisibleFaces)
	{
		visibleFaces = *preCaledVisibleFaces;
	}
	else
	{
		int hitFlags = 0;
	...
	Tab<int> lookup;
	lookup.SetCount(ld->GetNumberGeomVerts());
	...
	for (int i = 0; i < visibleFaces.GetSize(); i++)
	{
		...
		for (int j = 0; j < deg; j++)
		{
			...
			if (lookup[index] != -1)
			{
				Point3 p = ld->GetGeomVert(index) * toView;
...
int PeltData::HitTestEdgeMode(UnwrapMod *mod, MeshTopoData *ld, ViewExp& vpt, GraphicsWindow *gw, Tab<UVWHitData> &hitEdgesOutput, HitRegion hr, INode* inode, ModContext *mc, int flags, int type, const BitArray *preCaledVisibleFaces)
{
	if (!vpt.IsAlive())
	{
		// why are we here
	...
	if (type == HITTYPE_POINT)
	{
		...
		for (int hi = 0; hi < hitEdges.Count(); hi++)
		{
			...
			a = ld->GetGeomEdgeVert(eindex, 1);

			p = ld->GetGeomVert(a) * toView;
			if ((p.z > closest) || (closestIndex == -1))
			{
				closest = p.z;
				closestIndex = hi;
	...
	if (ld->mSeamEdges.GetSize() != ld->GetNumberGeomEdges())
	{
		ld->mSeamEdges.SetSize(ld->GetNumberGeomEdges());
			...
			if (mod->fnGetBackFaceCull())
			{
				if (selFace && visibleFace)
				{
						...
						!(flags&(HIT_UNSELONLY | HIT_SELONLY)))
					{
						...
						hitEdgesOutput.Append(1, &hitEdges[hi], 1);
					}
	...
	return hitEdgesOutput.Count();
}

void PeltData::ResolvePatchHandles()
{
	MeshTopoData *ld = mBaseMeshTopoDataCurrent;
	...
	for (int i = 0; i < ld->GetNumberTVEdges(); i++)
	{
		...
		int va = ld->GetTVEdgeVec(i, 0);
		int vb = ld->GetTVEdgeVec(i, 1);
		int a = ld->GetTVEdgeVert(i, 0);
		int b = ld->GetTVEdgeVert(i, 1);

		//make them 1/3 way between the end points
		if (ld->GetTVVertSelected(a) || ld->GetTVVertSelected(b))
		{
			Point3 pa, pb;
			...
			Point3 vec = (pb - pa)*0.33f;
			if (va != -1)
			{
				ld->SetTVVert(t, va, pa + vec);
			}
			if (vb != -1)
			{
				ld->SetTVVert(t, vb, pb - vec);
			}
		}
	}
}
...
void PeltData::RelaxRig(int iterations, float str, BOOL lockBoundaries)
{
	Tab<Point3> op;

	MeshTopoData *ld = mBaseMeshTopoDataCurrent;

	op.SetCount(ld->GetNumberTVVerts());
	str = str * 0.05f;
	TimeValue t = GetCOREInterface()->GetTime();
	...
	for (int i = 0; i < iterations; i++)
	{
		...
		for (int j = 0; j < rigPoints.Count(); j++)
		{
			...
			if (ld->GetTVVertSelected(selID))
			{
				if (!lockBoundaries)
				{
					ld->SetTVVert(t, selID, newPoint);
				}
				else
				{
					if (ld->GetTVVertSelected(rigPoints[prevID].lookupIndex) &&
						ld->GetTVVertSelected(rigPoints[nextID].lookupIndex))
					{
						ld->SetTVVert(t, selID, newPoint);
					}
				}
