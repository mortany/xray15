...
void TweakMouseProc::AverageUVW()
{
	if (mHitLD)
	{
		...
		if (ct > 0)
		{
			...
			mFinalUVW = uvw;
			mHitLD->SetTVVert(GetCOREInterface()->GetTime(),mHitTVVert,mFinalUVW);
	...
	case MOUSE_ABORT:
		{
			...
			if (mHitLD)
			{
				mHitLD->SetTVVert(GetCOREInterface()->GetTime(),mHitTVVert,mSourceUVW);
...
/*-------------------------------------------------------------------*/
TweakMode::TweakMode(UnwrapMod* bmod, IObjParam *i) :
