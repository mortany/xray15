...
void MeshTopoData::SetCache(MNMesh &mesh, int mapChannel)
{
	...
	if ( (mesh.selLevel==MNM_SL_FACE) && (s.NumberSet() == 0) )
	{
		ResetSelectionAndTVMapData();
	}
	...
	if (mesh.selLevel!=MNM_SL_FACE) 
	{
		...
		if (tVerts)
		{
			...
			ResizeTVVertSelection(mesh.M(mapChannel)->VNum());
			mVSelPreview.SetSize(mesh.M(mapChannel)->VNum());
		}
		else
		{
			TVMaps.v.SetCount(mesh.VNum());
			ResizeTVVertSelection(mesh.VNum());
			mVSelPreview.SetSize(mesh.VNum());
		}
		...
		for (int j=0; j<TVMaps.f.Count(); j++) 
		{
			...
			if (tvFace == NULL)
			{
				...
				TVMaps.f[j]->SetMatID(mesh.f[j].material);
				TVMaps.f[j]->ClearMatIDSet();
				TVMaps.f[j]->flags = 0;
	...
	else
	{
		...
		ResizeTVVertSelection(tvVertCount);
		mVSelPreview.SetSize(tvVertCount);
...
void MeshTopoData::BuildInitialMapping(MNMesh *msh)
{
	...
	ResizeTVVertSelection(usedIndex.NumberSet());
	mVSelPreview.SetSize(usedIndex.NumberSet());
...
void MeshTopoData::CopyMatID(MNMesh &mesh)
{
	...
	for (int k=0; k<tvFaceCount; k++) 
	{
		if (k < fCount)
		{
			if (TVMaps.f[k]->IsMatIDSet())
				mesh.f[k].material = TVMaps.f[k]->GetMatID();
		}
	}
}
void MeshTopoData::ApplyMapping(MNMesh &mesh, int mapChannel)
{
	...
	if (mesh.selLevel!=MNM_SL_FACE) 
	{
		...
		for (int k=0; k<tvFaceCount; k++) 
		{
			if (k < TVMaps.f.Count())
			{
				for (int m = 0; m< TVMaps.f[k]->count && m < tvFace[k].deg; m++)
				{
...
void MeshTopoData::ApplyHide(MNMesh &mesh)
{
	for (int i = 0; i < mGeomVertexFilter.GetSize(); i++)
	{
		mesh.v[i].SetFlag (MN_HIDDEN, !DoesGeomVertexPassFilter(i));
	}
	for (int i = 0; i < mFaceFilter.GetSize(); i++)
	{
		mesh.f[i].SetFlag (MN_HIDDEN, !DoesFacePassFilter(i));
	}
}
