...
void  MeshTopoData::RelaxVerts2(int subobjectMode, float relax, int iter, BOOL boundary, BOOL saddle, PeltData& peltData, BOOL applyToAll, BOOL updateUI)
{
	...
	else if (patch)
	{
		...
		RelaxPatch(subobjectMode, iter, relax, boundary);
		if (rd) delete rd;
		return;
	}
	...
	if (mesh) {
		...
		for (int k = 0; k < iter; k++)
		{
			...
			if ((k % 10) == 0)
			{
				...
				if (updateUI)
				{
					RaiseNotifyUpdateTexmap();
				}
				RaiseNotifyUIInvalidation(FALSE);
			}
	...
	if (peltData.peltDialog.hWnd)
		peltData.RelaxRig(iter, relax, boundary);
}

void MeshTopoData::RelaxPatch(int subobjectMode, int iteration, float str, BOOL lockEdges)
{
	TimeValue t = GetCOREInterface()->GetTime();
	...
	if (lockEdges)
	{
		...
		for (int j = 0; j < TVMaps.f.Count(); j++)
		{
			...
			for (int k = 0; k < count; k++)
			{
				...
				if (GetTVVertSelected(index) || TVMaps.v[index].GetInfluence() > 0.0f)
					selCount++;
			}
			if (selCount == count)
				tempFSel.Set(j);
...
void MeshTopoData::Relax(int subobjectMode, int iteration, float str, BOOL lockEdges, BOOL matchArea)
{
	// get the number of polyons
	int faceCount = TVMaps.f.Count();
	if (faceCount == 0) return;
...
void MeshTopoData::RelaxFit(int subobjectMode, int iteration, float str)
{
	//get scale
...
void MeshTopoData::RelaxByFaceAngle(int subobjectMode, int frames, float stretch, float str, BOOL lockEdges, HWND statusHWND, PeltData& peltData, float aspect, BOOL applyToAll)
{
	...
	BitArray rigPoint;
	MeshTopoData *peltLD = peltData.mBaseMeshTopoDataCurrent;
	if (peltData.peltDialog.hWnd)
	{
		if (peltLD == this)
		{
			rigPoint.SetSize(peltLD->GetNumberTVVerts());
			rigPoint.ClearAll();
	...
	if (ct)
	{
		...
		for (int cframe = 0; cframe < frames; cframe++)
		{
			...
			for (int i = 0; i < TVMaps.f.Count(); i++)
			{
				...
				if (useFace[i])
				{
					...
					rvec = Normalize(rvec);
					float fangle = AngleFromVectors(ovec, rvec);
					tm.RotateZ(fangle);
			...
			for (int i = 0; i < TVMaps.v.Count(); i++)
			{
				...
				if ((w > 0.0f) && (!(TVMaps.v[i].IsFrozen())) && !isRigPoint && deltaCount[i])
				{
					Point3 uvP = TVMaps.v[i].GetP();
					if (lockEdges)
					{

						if (!edgeVerts[i])
							TVMaps.v[i].SetP(uvP + ((deltaList[i] / (float)deltaCount[i]) * w * str));
					}
					else TVMaps.v[i].SetP(uvP + ((deltaList[i] / (float)deltaCount[i]) * w * str));
				}
			}

			if ((cframe % 10) == 0)
			{
				peltData.ResolvePatchHandles();
				if (statusHWND)
				{
					RaiseNotifyUpdateTexmap();
				}
				RaiseNotifyUIInvalidation(statusHWND ? TRUE : FALSE);
			}
		}
		...
		if (peltData.peltDialog.hWnd && !applyToAll)
		{
			peltData.RelaxRig(frames, str, lockEdges);
		}
...
void MeshTopoData::RelaxByEdgeAngle(int subobjectMode, int frames, float stretch, float str, BOOL lockEdges, HWND statusHWND, PeltData& peltData, BOOL applyToAll)
...
void  MeshTopoData::RelaxBySprings(int subobjectMode, int frames, float stretch, BOOL lockEdges, PeltData& peltData)
{
	float reducer = 1.0f;
	...
	{
		float wa = 0.0f;
		if (GetTVVertSelected(i)) wa = 1.0f;
	...
	for (int i = 0; i < TVMaps.f.Count(); i++)
	{
		...
		for (int j = 0; j < deg; j++)
		{
			...
			float wa = 0.0f;
			if (GetTVVertSelected(a)) wa = 1.0f;
			else wa = TVMaps.v[a].GetInfluence();
			if (wa > 0.0f) ct++;
		}
		if (ct != deg)
	...
	for (int i = 0; i < TVMaps.ePtrList.Count(); i++)
	{
		...
		if (GetTVVertSelected(b)) wb = 1.0f;
		else wb = TVMaps.v[b].GetInfluence();
		if ((wa > 0.0f) && (wb > 0.0f))
		{
			if (lockEdges)
