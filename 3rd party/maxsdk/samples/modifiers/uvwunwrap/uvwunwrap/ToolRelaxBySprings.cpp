...
void UnwrapMod::fnRelaxByFaceAngle(int frames, float stretch, float str, BOOL lockEdges,HWND statusHWND)
{
	...
	bRelaxFinished = FALSE;
	if (fnGetNonSquareApplyBitmapRatio())
	{
		//Restore the data
		for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
		{
			mMeshTopoData[ldID]->NonSquareAdjustVertexY(aspect, 0.5, true);
		}
	}	

	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxByFaceAngle(fnGetTVSubMode(), frames, stretch, str, lockEdges, statusHWND,this->peltData, this->aspect, applyToAll);
	...
	if (fnGetNonSquareApplyBitmapRatio())
	{
		...
		for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
		{
			mMeshTopoData[ldID]->NonSquareAdjustVertexY(aspect, 0.5, false);
		}
	}	
	bRelaxFinished = TRUE;
...
void UnwrapMod::fnRelaxByEdgeAngle(int frames, float stretch,float str, BOOL lockEdges,HWND statusHWND)
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxByEdgeAngle(fnGetTVSubMode(), frames, stretch, str, lockEdges, statusHWND, this->peltData, applyToAll);
		if (mMeshTopoData[ldID]->GetUserCancel())
			ldID = mMeshTopoData.Count();
	}
...
//    int frames = 10;
 //	float stretch = 0.87f;
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxBySprings(mTVSubObjectMode, frames, stretch, lockEdges, this->peltData);
	}
