...
void MeshTopoData::ResetSelectionAndTVMapData()
{
	TVMaps.SetCountFaces(0);
	TVMaps.v.SetCount(0);
	TVMaps.FreeEdges();
	TVMaps.FreeGeomEdges();
	ResizeTVVertSelection(0);
	mVSelPreview.SetSize(0);
	mESel.SetSize(0);
	mESelPreview.SetSize(0);
	mFSel.SetSize(0);
	mFSelPreview.SetSize(0);
	mGESel.SetSize(0);
	mGVSel.SetSize(0);
}
...
void MeshTopoData::SetCache(Mesh &mesh, int mapChannel)
{
	...
	if (mesh.selLevel!=MESH_FACE) 
	{
		...
		ResizeTVVertSelection(mesh.getNumMapVerts(mapChannel));
		ClearTVVertSelection();

		mVSelPreview.SetSize(mesh.getNumMapVerts(mapChannel));
		...
		for (int j=0; j<TVMaps.f.Count(); j++) 
		{
			...
			if (tvFace == NULL)
			{
				...
				TVMaps.f[j]->SetMatID(mesh.faces[j].getMatID());
...
void MeshTopoData::CopyMatID(Mesh &mesh)
{
	int tvFaceCount = mesh.getNumFaces();
	...
	for (int k=0; k<tvFaceCount; k++) 
	{
		if (k < fCount)
		{
			if (TVMaps.f[k]->IsMatIDSet())
				mesh.faces[k].setMatID( TVMaps.f[k]->GetMatID() );
		}
	}
}
...
void MeshTopoData::ApplyHide(Mesh &mesh)
{
	for (int i = 0; i < mFaceFilter.GetSize(); i++)
	{
		mesh.faces[i].SetHide(!DoesFacePassFilter(i));
	}
}