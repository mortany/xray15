...
void MeshTopoData::SetCache(PatchMesh &patch, int mapChannel)
{
	...
	if (patch.selLevel!=PATCH_PATCH ) 
	{
		...
		ResizeTVVertSelection(patch.getNumMapVerts(mapChannel));
		mVSelPreview.SetSize(patch.getNumMapVerts (mapChannel));
		...
		for (int j=0; j<TVMaps.f.Count(); j++) 
		{
			...
			if (tvFace == NULL)
			{
				...
				TVMaps.f[j]->SetMatID(patch.getPatchMtlIndex(j));
	...
	else
	{
		...
		for (int j=0; j<TVMaps.f.Count(); j++) 
		{
			...
			if (tvFace == NULL)
			{
				...
				if (pcount == 4) TVMaps.f[j]->t[3] = 0;
				TVMaps.f[j]->FaceIndex = j;
				TVMaps.f[j]->SetMatID(patch.patches[j].getMatID());
				TVMaps.f[j]->ClearMatIDSet();
				if (patch.patchSel[j])
					TVMaps.f[j]->flags = 0;
...
void MeshTopoData::CopyMatID(PatchMesh &patch)
{
	int tvFaceCount = patch.getNumPatches();
	...
	for (int k=0; k<tvFaceCount; k++) 
	{
		if (k < fCount)
		{
			if (TVMaps.f[k]->IsMatIDSet())
				patch.setPatchMtlIndex(k, TVMaps.f[k]->GetMatID());
		}
	}
}
...
void MeshTopoData::ApplyHide(PatchMesh &patch)
{
	for (int i = 0; i < mFaceFilter.GetSize(); i++)
	{
		patch.patches[i].SetHidden(!DoesFacePassFilter(i));
	}
}
