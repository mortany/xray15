...
static FPInterfaceDesc unwrap_interface6(
	...
	unwrap_splinemap_getCrossSection_Pos, _T("splineMap_GetCrossSection_Pos"), 0, TYPE_POINT3_BV, 0, 2,
	_T("splineIndex"), 0, TYPE_INDEX,
	_T("crossSectionIndex"), 0, TYPE_INDEX,
	...
	_T("splineIndex"), 0, TYPE_INDEX,
	_T("crossSectionIndex"), 0, TYPE_INDEX,
	unwrap_splinemap_getCrossSection_Twist, _T("splineMap_GetCrossSection_Twist"), 0, TYPE_QUAT_BV, 0, 2,
	_T("splineIndex"), 0, TYPE_INDEX,
	_T("crossSectionIndex"), 0, TYPE_INDEX,
	...
	unwrap_alignbypivot_horizontal, _T("alignByPivotHorizontal"), 0, TYPE_VOID, 0, 0,
	unwrap_alignbypivot_vertical, _T("alignByPivotVertical"), 0, TYPE_VOID, 0, 0,

	unwrap_paintview, _T("paintview"), 0, TYPE_VOID, 0, 0,
	unwrap_close_editor, _T("closeEditor"), 0, TYPE_VOID, 0,0,
	unwrap_close_options, _T("closeOptions"), 0, TYPE_VOID, 0, 0,
	unwrap_relaxthreaded, _T("relaxThreaded"), 0, TYPE_VOID, 0, 1,
		_T("threadOp"), 0, TYPE_ENUM, relax_threaded_enum,

	properties,
	unwrap_get_mirror_selection_status, unwrap_set_mirror_selection_status, _T("MirrorGeomSelection"), 0, TYPE_BOOL,
	unwrap_get_mirror_axis, unwrap_set_mirror_axis, _T("MirrorSelectionAxis"), 0, TYPE_ENUM, mirror_axis_enum,
	unwrap_get_mirror_threshold, unwrap_set_mirror_threshold, _T("MirrorSelectionThreshold"), 0, TYPE_FLOAT,
	unwrap_get_selection_preview, unwrap_set_selection_preview, _T("SelectionPreview"), 0, TYPE_BOOL,
	unwrap_get_paint_move_brush, unwrap_set_paint_move_brush, _T("PaintMoveBrush"), 0, TYPE_bool,
	unwrap_get_relax_move_brush, unwrap_set_relax_move_brush, _T("RelaxMoveBrush"), 0, TYPE_bool,
	unwrap_get_paint_full_strength_size, unwrap_set_paint_full_strength_size, _T("PaintFullStrength"), 0, TYPE_FLOAT,
	unwrap_get_paint_fall_off_size, unwrap_set_paint_fall_off_size, _T("PaintFallOff"), 0, TYPE_FLOAT,
	unwrap_get_paint_falloff_type, unwrap_set_paint_falloff_type, _T("PaintFallOffType"), 0, TYPE_ENUM, paint_falloff_type_enum,
	unwrap_get_typein_linkUV, unwrap_set_typein_linkUV, _T("TypeInLinkUV"), 0, TYPE_bool,
	unwrap_get_editor_hwnd, FP_NO_FUNCTION, _T("editorHWnd"), 0, TYPE_HWND,
	unwrap_get_editor_view_hwnd, FP_NO_FUNCTION, _T("editorViewHWnd"), 0, TYPE_HWND,

	unwrap_get_tile_grid_visible, unwrap_set_tile_grid_visible, _T("showTileGridlines"), 0, TYPE_BOOL,
	unwrap_get_display_pixel_units, unwrap_set_display_pixel_units, _T("DisplayPixelUnits"), 0, TYPE_BOOL,
	...
	mirror_axis_enum, 3,
	_T("axisX"), mirror_by_x_axis,
	_T("axisY"), mirror_by_y_axis,
	_T("axisZ"), mirror_by_z_axis,
	paint_falloff_type_enum, 4,
	_T("linear"), paint_falloff_linear,
	_T("smooth"), paint_falloff_smooth,
	_T("slow"), paint_falloff_slow,
	_T("fast"), paint_falloff_fast,
	relax_threaded_enum, 3,
		_T("start"), UnwrapMod::KThreadStart,
		_T("restart"), UnwrapMod::KThreadReStart,
		_T("stop"), UnwrapMod::KThreadEnd,
	p_end
	);
...
static ParamBlockDesc2 unwrap_param_blk(unwrap_params, _T("params"), 0, &unwrapDesc,
	...
	p_default, TRUE,
	p_ui, unwrap_renderuv_params, TYPE_SINGLECHECKBOX, IDC_VISIBLE_CHECK,
	p_end,
	unwrap_renderuv_invisible, _T("renderuv_invisibleedges"), TYPE_BOOL, 0, IDS_INVISIBLEEDGES,
	p_default, FALSE,
	p_ui, unwrap_renderuv_params, TYPE_SINGLECHECKBOX, IDC_INVISIBLE_CHECK,
	p_end,

	unwrap_renderuv_seamedges, _T("renderuv_seamedges"), TYPE_BOOL, 0, IDS_SEAMEDGES,
	p_default, TRUE,
	p_ui, unwrap_renderuv_params, TYPE_SINGLECHECKBOX, IDC_SEAM_CHECK,
	p_end,
	...
	p_default, TRUE,
	p_ui, unwrap_renderuv_params, TYPE_SINGLECHECKBOX, IDC_FORCE2SIDE_CHECK,
	p_end,
	...
	p_default, TRUE,
	p_ui, unwrap_renderuv_params, TYPE_SINGLECHECKBOX, IDC_OVERLAP_CHECK,
	p_end,
	...
	unwrap_floaters_DEPRECATED, _T("floaters"), TYPE_STRING_TAB, 0, P_OBSOLETE | P_VARIABLE_SIZE, 0,
	p_end,
	unwrap_floaters_visible_DEPRECATED, _T("floatersVisible"), TYPE_BOOL_TAB, 0, P_OBSOLETE | P_VARIABLE_SIZE, 0,
	p_end,
	...
	unwrap_texturecheckermtl,	_T("TextureCheckerMaterial"), 		TYPE_MTL, 	0, 	0, 
	p_end,

	p_end
	);

const static float sDefaultMirrorThreshold = 0.01f;
bool UnwrapMod::sSelectionPreview = true;
bool UnwrapMod::sPeelDetach = true;
bool UnwrapMod::sAutoPack = true;
float UnwrapMod::sPaintFallOffSize = 20.0f;
float UnwrapMod::sPaintFullStrengthSize = 10.0f;
int	UnwrapMod::sPaintFallOffType = paint_falloff_linear;

void UnwrapMod::OnModSelChanged(void *param, NotifyInfo *info)
{
	UnwrapMod *mod = (UnwrapMod*)param;
	if (mod)
		mod->bMeshTopoDataContainerDirty = TRUE;
}

UnwrapMod::UnwrapMod() :
	mirrorGeoSelectionAxis(mirror_by_x_axis),
	mirrorGeoSelectionStatus(FALSE),
	mirrorGeoThreshold(sDefaultMirrorThreshold),
	bSuspendSelectionSync(FALSE),
	bMeshTopoDataContainerDirty(TRUE), bShowFPSinEditor(FALSE), bMapGizmoTMDirty(TRUE)
{
	
	RegisterNotification(OnModSelChanged, this, NOTIFY_MODPANEL_SEL_CHANGED);
	...
	if (GetCOREInterface() && GetCOREInterface()->GetMenuManager()->RegisterMenuBarContext(kUnwrapMenuBar, _T("UVW Unwrap Menu Bar")))
	{
	...
	packMethod = unwrap_non_convex_pack;
	packSpacing = 0.001f;
	packNormalize = TRUE;
	packRotate = TRUE;
	packFillHoles = FALSE;
	...
	paintSize = MaxSDK::UIScaled(8);

	sketchCursorSize = MaxSDK::UIScaled(8);
	tickSize = MaxSDK::UIScaled(2);
	...
	COLORREF peelColor = RGB(181, 109, 218);
	name.printf(_T("%s"), GetString(IDS_PEEL_COLOR));
	category.printf(_T("%s"), GetString(IDS_PW_UNWRAPCOLOR));
	iret = ColorMan()->RegisterColor(PEELCOLORID, name, category, peelColor);
	...
	{
		TCHAR cfgFile[1024];
		_tcscpy_s(cfgFile, _countof(cfgFile), GetCOREInterface()->GetDir(APP_PLUGCFG_DIR));
	...
	mDistortionOption = eUndefined;
	showMultiTile = FALSE;

	freeFromViewValidate = TRUE;
	freeFormMoveAxisLength = 120;

	mTypeInLinkUV = true;
	freeFormMoveAxisLocked = eMoveBoth;

	fCheckerTiling = CHECKERTILINGDEFAULT;
	bRelaxFinished = TRUE;

	mNonSquareApplyBitmapRatio = TRUE;
...
static FPInterfaceDesc unwrap_interface(
	...
	unwrap_snaptoggle, _T("snapToggle"), 0, TYPE_VOID, 0, 0,
...
static FPInterfaceDesc unwrap_interface2(
	...
	unwrap_unfoldmap, _T("unfoldMap"), 0, TYPE_VOID, 0, 2,
	_T("unfoldMethod"), 0, TYPE_INT,
	_T("normalized"), 0, TYPE_BOOL,
	...
	unwrap_getwindowx, _T("getWindowX"), 0, TYPE_INT, 0, 1,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_getwindowy, _T("getWindowY"), 0, TYPE_INT, 0, 1,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_getwindoww, _T("getWindowW"), 0, TYPE_INT, 0, 1,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_getwindowh, _T("getWindowH"), 0, TYPE_INT, 0, 1,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	unwrap_setselectionmatid, _T("setSelectionMatID"), 0, TYPE_VOID, 0, 1,
	_T("matID"), 0, TYPE_INT,
	...
	unwrap_getpixelsnap, _T("getPixelSnap"), 0, TYPE_BOOL, 0, 0,
	unwrap_setpixelsnap, _T("setPixelSnap"), 0, TYPE_VOID, 0, 1,
	_T("pixelSnap"), 0, TYPE_BOOL,
	...
	unwrap_setdisplayopenedge, _T("setDisplayOpenEdges"), 0, TYPE_VOID, 0, 1,
	...
	unwrap_selectverttoedge, _T("vertToEdgeSelect"), 0, TYPE_VOID, 0, 1, _T("partialSelect"), 0, TYPE_BOOL, f_keyArgDefault, FALSE,
	unwrap_selectverttoface, _T("vertToFaceSelect"), 0, TYPE_VOID, 0, 1, _T("partialSelect"), 0, TYPE_BOOL, f_keyArgDefault, FALSE,

	unwrap_selectedgetovert, _T("edgeToVertSelect"), 0, TYPE_VOID, 0, 0,
	unwrap_selectedgetoface, _T("edgeToFaceSelect"), 0, TYPE_VOID, 0, 1, _T("partialSelect"), 0, TYPE_BOOL, f_keyArgDefault, FALSE,
	...
	unwrap_gethitsize, _T("getHitSize"), 0, TYPE_INT, 0, 1,
	_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_sethitsize, _T("SetHitSize"), 0, TYPE_VOID, 0, 2,
	_T("size"), 0, TYPE_INT,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	unwrap_getfiltermap, _T("getFilterMap"), 0, TYPE_BOOL, 0, 0,
	unwrap_setfiltermap, _T("setFilterMap"), 0, TYPE_VOID, 0, 1,
	_T("filter"), 0, TYPE_BOOL,
	...
	unwrap_getpaintsize, _T("getPaintSelectSize"), 0, TYPE_INT, 0, 1,
	_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_setpaintsize, _T("setPaintSelectSize"), 0, TYPE_VOID, 0, 2,
	_T("size"), 0, TYPE_INT,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	unwrap_getticksize, _T("getTickSize"), 0, TYPE_INT, 0, 1,
	_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_setticksize, _T("setTickSize"), 0, TYPE_VOID, 0, 2,
	_T("size"), 0, TYPE_INT,
	_T("applyUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	unwrap_getsnaptoggle, _T("getSnapToggle"), 0, TYPE_BOOL, 0, 0,
	unwrap_setsnaptoggle, _T("setSnapToggle"), 0, TYPE_VOID, 0, 1,
	_T("snapToggle"), 0, TYPE_BOOL,
	...
	unwrap_getsnapstrength, _T("getSnapStrength"), 0, TYPE_FLOAT, 0, 0,
	unwrap_setsnapstrength, _T("setSnapStrength"), 0, TYPE_VOID, 0, 1,
	_T("snapStrength"), 0, TYPE_FLOAT,
...
static FPInterfaceDesc unwrap_interface5(
	...
	unwrap_setwindowxoffset, _T("setWindowXOffset"), 0, TYPE_VOID, 0, 2,
	_T("offset"), 0, TYPE_INT,
	_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	unwrap_setwindowyoffset, _T("setWindowYOffset"), 0, TYPE_VOID, 0, 2,
	_T("offset"), 0, TYPE_INT,
	_T("removeUIScaling"), 0, TYPE_bool, f_keyArgDefault, true,
	...
	unwrap_renderuv, _T("renderUV"), 0, TYPE_VOID, 0, 3,
	_T("fileName"), 0, TYPE_FILENAME,
	_T("UTile"), 0, TYPE_INT, f_keyArgDefault, 1,
	_T("VTile"), 0, TYPE_INT, f_keyArgDefault, 1,
	...
	unwrap_getshowmap, _T("getShowMultiTile"), 0, TYPE_BOOL, 0, 0,
	unwrap_setshowmap, _T("setShowMultiTile"), 0, TYPE_VOID, 0, 1,
	_T("showMultiTile"), 0, TYPE_BOOL,
...
void UnwrapMod::fnCloseEditor()
{
	if (IsWindow(hDialogWnd))
	{
		//CloseWindow(hDialogWnd);
		SendMessage(hDialogWnd, WM_CLOSE, 0, 0);
	}
}

void UnwrapMod::fnCloseOptions()
{
	if (IsWindow(hOptionshWnd))
	{
		SendMessage(hOptionshWnd, WM_CLOSE, 0, 0);
	}
}
...
void UnwrapMod::fnEdit()
{
	...
	if (ip)
	{
		this->RebuildEdges(); // seems added for safety.
		if (!hDialogWnd)
		{
			...
			// set up quad menu and main menu bar
			IMenuBarContext* pContext = (IMenuBarContext*)GetCOREInterface()->GetMenuManager()->GetContext(kUnwrapMenuBar);
			assert(pContext);
			pContext->CreateWindowsMenu();
		...
		else
		{
			...
			ShowWindow(hDialogWnd, SW_RESTORE);
		}
	}
}
...
{
	fnSetFilteredSelected(!fnGetFilteredSelected());
}
...
{
	if (filter != filterSelectedFaces)
	{
		filterSelectedFaces = filter;
		BuildAllFilters();
		// if mirror selection is enabled, rebuild the mirror index table since some sub objs have been hidden/unhidden.
...
{
	//The CurrentMap internally begin from -1.
	return CurrentMap+2;
}

void UnwrapMod::fnSetCurrentMap(int map)
{
	//If the user set 1,it internally means -1(the texture checker)
	//If the user set 2,it internally means 0(the regular checker)
	--map;//script index to C++ index;
	if (map >= dropDownListIDs.Count() || map < 0)
		return;

	int newMap = dropDownListIDs[map];
	int ct = pblock->Count(unwrap_texmaplist); //fnNumberMaps();
	if (newMap >= -1 && newMap < ct)
	{
		CurrentMap = newMap;
		if (CurrentMap == TEXTURECHECKERINDEX)
		{
			ShowTextureCheckerMtl();
		}
...
{
	//If the dropdown list only contain the texture checker and regular checker and other fixed items,
	//the whole count is 10, so the maps count should be 10 - (7+1) = 2 that means the count of the texture checker and regular checker.
	//If the dropdown list add other textures, the whole count is 10 + n, so the maps count should be (10+n) - (7+1) = 2+n.
	//So the formula is : Count of the dropdown list - (AngleDistortionOffset+1).
	int ct = SendMessage( hTextures, CB_GETCOUNT, 0, 0 )-(AngleDistortionOffset+1);
	return ct;
}
...
Point3 *UnwrapMod::fnGetSelPreviewColor()
{
	AColor c(selPreviewColor);
	lColor.x = c.r;
	lColor.y = c.g;
	lColor.z = c.b;
	return &lColor;
...
void UnwrapMod::fnSetSelPreviewColor(Point3 color)
{
	AColor c;
	c.r = color.x;
	c.g = color.y;
...
BOOL UnwrapMod::fnGetPixelCenterSnape()
{
	return pixelCenterSnap;
}

void UnwrapMod::fnSetPixelCenterSnape(BOOL pixelCenter)
{
	pixelCenterSnap = pixelCenter;
}

BOOL UnwrapMod::fnGetPixelCornerSnap()
{
	return pixelCornerSnap;
}

void UnwrapMod::fnSetPixelCornerSnap(BOOL pixelCorner)
{
	pixelCornerSnap = pixelCorner;
}
...
void UnwrapMod::fnSelectPolygons(BitArray *sel, INode *node)
{
	...
	if (ld)
	{
		...
		for (int i = 0; i < ld->GetNumberFaces(); i++)
		{
			...
			{
				if ((*sel)[i])
					ld->SetFaceSelected(i, TRUE, fnGetMirrorSelectionStatus());//md->faceSel.Set(i);
			}
		}
		fnSyncTVSelection();
		NotifyDependents(FOREVER, PART_SELECT, REFMSG_CHANGE);
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		if (ld)
		{
			...
			if (ld->GetFaceSel().AnyBitSet())
			{
				for (int i = 0; i < ld->GetNumberFaces(); i++)
				{
					if (!ld->GetFaceDead(i) && ld->GetFaceSelected(i))
					{
						int degree = ld->GetFaceDegree(i);
						for (int j = 0; j < degree; j++)
						{
							int id = ld->GetFaceTVVert(i, j);
							tempSel.Set(id, TRUE);
						}
					}
				}
...
//helper method to remove a vertex from vertex selection
void RemoveVertexFromSelection(MeshTopoData* meshData, int vertIndex)
{
	//Clean up this dead vertex if its in the vertex selection
	auto vertSel = meshData->GetTVVertSel();
	if (vertSel[vertIndex])
	{
		//this dead vertex was selected, clear it
		BitArray tempSel;
		tempSel.SetSize(vertSel.GetSize());
		tempSel = vertSel;
		tempSel.Clear(vertIndex);
		meshData->SetTVVertSel(tempSel);
	}
}
...
void UnwrapMod::fnMarkAsDead(int index, INode *node)
{
	...
	if (ld)
	{
		...
		RemoveVertexFromSelection(ld, index);
	}
}
...
void UnwrapMod::fnAddPoint(Point3 pos, int fIndex, int vIndex, BOOL sel, INode *node)
{
	...
	if (ld)
	{
		ld->AddTVVert(0, pos, fIndex, vIndex, sel);
	}
}
...
void UnwrapMod::fnAddHandle(Point3 pos, int fIndex, int vIndex, BOOL sel, INode *node)
{
	...
	if (ld)
	{
		ld->AddTVHandle(0, pos, fIndex, vIndex, sel);
	}
}
...
void UnwrapMod::fnAddInterior(Point3 pos, int fIndex, int vIndex, BOOL sel, INode *node)
{
	...
	if (ld)
	{
		ld->AddTVInterior(0, pos, fIndex, vIndex, sel);
	}
}
...
BOOL	UnwrapMod::fnGetShowMultiTile()
{
	return showMultiTile;
}

void	UnwrapMod::fnSetShowMultiTile(BOOL smultiTile)
{
	showMultiTile = smultiTile;
	InvalidateView();
	mUIManager.UpdateCheckButtons();
}

void	UnwrapMod::fnShowMultiTile()
{
	showMultiTile = !showMultiTile;
	InvalidateView();
}

int		UnwrapMod::fnGetWindowX(bool removeUIScaling)
{
...
int		UnwrapMod::fnGetWindowY(bool removeUIScaling)
{
	WINDOWPLACEMENT floaterPos;
	GetWindowPlacement(hDialogWnd, &floaterPos);
...
int		UnwrapMod::fnGetWindowW(bool removeUIScaling)
{
	WINDOWPLACEMENT floaterPos;
	...
	{
		int res = floaterPos.rcNormalPosition.right - floaterPos.rcNormalPosition.left;
		if (removeUIScaling)
			res = MaxSDK::UIUnScaled(res);
		return res;
	}
...
int		UnwrapMod::fnGetWindowH(bool removeUIScaling)
{
	WINDOWPLACEMENT floaterPos;
	floaterPos.length = sizeof(WINDOWPLACEMENT);
	GetWindowPlacement(hDialogWnd, &floaterPos);
	...
	{
		int res = floaterPos.rcNormalPosition.bottom - floaterPos.rcNormalPosition.top;
		if (removeUIScaling)
			res = MaxSDK::UIUnScaled(res);
		return res;
	}
...
{
	if (freeFormMode != fnGetFreeFormMode())
	{
		if (freeFormMode)
		{
...
void	UnwrapMod::UpdateShowSharedTVEdges(MeshTopoData* pData)
{
	if ((showVertexClusterList || fnGetShowShared()) && pData)
	{
		pData->UpdateSharedTVEdges(fnGetTVSubMode());
	}
}
...
BOOL	UnwrapMod::fnGetFilterMap()
{
	return filterMap;
}

void	UnwrapMod::fnSetFilterMap(BOOL filter)
{
	filterMap = filter;
	tileValid = FALSE;
	InvalidateView();
...
int		UnwrapMod::fnGetTickSize(bool removeUIScaling)
{
	int res = tickSize;
	if (removeUIScaling)
		res = MaxSDK::UIUnScaled(res);
...
void	UnwrapMod::fnSetTickSize(int size, bool applyUIScaling)
{
	if (applyUIScaling)
		size = MaxSDK::UIScaled(size);
	tickSize = size;
	if (tickSize < MaxSDK::UIScaled(1)) tickSize = MaxSDK::UIScaled(1);
	InvalidateView();
}
...
BOOL	UnwrapMod::fnGetSnapToggle()
{
	return snapToggle;
}

void	UnwrapMod::fnSetSnapToggle(BOOL snap)
{
	if (snap != snapToggle)
	{
		snapToggle = snap;
...
float	UnwrapMod::fnGetSnapStrength()
{
	return snapStrength * 2.0f;
}

void	UnwrapMod::fnSetSnapStrength(float str)
{
	str = str *0.5f;
...
{
	if (relative == absoluteTypeIn)
	{
		absoluteTypeIn = !relative;
		SetupTypeins();
		mUIManager.UpdateCheckButtons();
	}
}
...
void UnwrapMod::fnPeltDialogRun()
{
	theHold.Begin();
	HoldPoints();
	theHold.Put(new UnwrapPeltVertVelocityRestore(this->peltData));
	peltData.Run(this);
...
{
	if (samples != fnGetPeltDialogSamples())
	{ 
...
{
	if (strength != fnGetPeltDialogRigStrength())
	{
...
{
	if (stiffness != fnGetPeltDialogStiffness())
	{
...
{
	if (dampening != fnGetPeltDialogDampening())
	{
...
{
	// note: calling SetMapMode(mode) twice actually turn off the mode.
	// act like a toggle.
...
void UnwrapMod::fnQMap()
{
	...
	ApplyQMap();
	if (!holding)
		theHold.Accept(GetString(IDS_QMAP));
}
...
void UnwrapMod::ApplyQMap()
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		if (abs(aspect - 1.0) > aspectThreshold)
		{
			Point3 s(1.0f, aspect, 1.0f);
			tm.Scale(s);
		}

		MeshTopoData *ld = mMeshTopoData[ldID];
		ld->ApplyMap(PLANARMAP, fnGetNormalizeMap(), tm, GetFilterMatID());
	}
	mapMapMode = holdMap;
	NotifyDependents(FOREVER, PART_TEXMAP, REFMSG_CHANGE);
	if (ip)
...
int	UnwrapMod::fnGetMatIDSelect()
{
	...
	{
		return iSetMatIDSpinnerEdit->GetIVal();
	}
	return 0;
}
...
void	UnwrapMod::fnSetMatIDSelect(int matID)
{
	...
	if (iSetMatIDSpinnerEdit)
	{
		iSetMatIDSpinnerEdit->SetValue(matID, TRUE);
	}
	//perform the actual work of setting the material ID for the selected faces.
	fnSetSelectionMatID(matID);
}
...
void	UnwrapMod::fnUVLoop(int mode)
{
		...
		for (int i = 0; i < GetMeshTopoDataCount(); i++)
		{
			...
			for (int i = 0; i < ld->GetNumberTVEdges(); i++)
			{
				...
				else
				{
					...
					if (sel && !unsel)//so the edge is not on border, skip
					{
						skipsel.Set(i, TRUE);
			...
			for (int i = 0; i < ld->GetNumberTVEdges(); i++)
			{
				if (esel[i] && !skipsel[i])
				{
					int tid1 = ld->GetTVEdgeVert(i, 0);
					int tid2 = ld->GetTVEdgeVert(i, 1);
					if (!vsel[tid1] && !vsel[tid2]) // this conditon cause the factor that the selected face must be continous so that their borders may be selected edges.
					{
						esel.Clear(i);
					}
				}
	...
	if (mode == 1)
		...
		ShrinkUVLoop();
	else //mode 0, to select the loop of all the selected edges
	{
		SelectUVEdge(TRUE);
	}
...
void	UnwrapMod::SpaceOrAlign(int align, BOOL horizontal)
{
	...
	//see if we need to auto pin
	if (pblock->GetInt(unwrap_autopin) && GetLivePeelModeEnabled())
	{
		if (fnGetTVSubMode() == TVVERTMODE)  //in vert mode we can just pin selected
		{
			for (int i = 0; i < GetMeshTopoDataCount(); i++)
			{
		...
		//edge mode needs to convert to selection to vertex and then mark as pin
		else if (fnGetTVSubMode() == TVEDGEMODE)
		{
			for (int i = 0; i < GetMeshTopoDataCount(); i++)
			{
	...
	//if we are in live peel update the solve
	if (GetLivePeelModeEnabled())
	{
...
void UnwrapMod::RescaleSelectedCluster(BOOL bOnlySelected)
{
	theHold.Begin();
	HoldPoints();
	theHold.Accept(GetString(IDS_PW_RESCALECLUSTER));

	bool emptySelections = true;
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		MeshTopoData *ld = mMeshTopoData[ldID];
		BitArray faceSel = ld->GetFaceSel();
		...
		{
			BitArray vselTemp = ld->GetTVVertSel();
			BitArray vsel;
			ld->GetVertSelFromEdge(vsel);
			ld->SetTVVertSel(vsel);
			ld->GetFaceSelFromVert(faceSel, FALSE);
			ld->SetTVVertSel(vselTemp);
		}
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		if (emptySelections || bOnlySelected == FALSE)
		{
			faceSel.SetAll();
		}
...
void UnwrapMod::fnRescaleCluster(BitArray *sel, INode *node)
{
	...
	if (ld && sel)
	{
		ConstantTopoAccelerator topoAccl(ld, TRUE);
		...
		BitArray usedFaces(ld->GetNumberFaces());
		...
		for (int i = 0; i < sel->GetSize(); i++)
		{
			if ((*sel)[i] && !usedFaces[i] && i < ld->GetNumberFaces() && ld->DoesFacePassFilter(i))
			{
				ld->ClearFaceSelection();
				ld->SetFaceSelected(i, TRUE);
				ld->SelectElement(TVFACEMODE, TRUE);
				BOOL clusterAdded = FALSE;
				int groupID = ld->GetToolGroupingData()->GetGroupID(i);
				bool inGroup = (groupID > -1);
				for (int j = 0; j < ld->GetNumberFaces(); j++)
				{
					if (ld->GetFaceSelected(j) || (inGroup && ld->GetToolGroupingData()->GetGroupID(j)==groupID) 
						&& !usedFaces[j] && clusterIDs[j] == -1)
					{
		...
		ld->SetFaceSelectionByRef(holdFaceSel);
		ld->RescaleClusters(clusterIDs, clustRescale);
...
void UnwrapMod::fnRelaxOneClick()
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		...
		while (timePassed < timeLimit)
		{
			...
			BitArray mVSelTemp = mMeshTopoData[ldID]->GetTVVertSel();
			if (applyToAll)
			{
				mVSelTemp.ClearAll();
			...
			if (ct % 2 == 0)
				mMeshTopoData[ldID]->RelaxByFaceAngle(fnGetTVSubMode(), 20, rstretch, rstrength, 0, NULL, this->peltData, this->aspect, applyToAll);
			else
				mMeshTopoData[ldID]->RelaxByEdgeAngle(fnGetTVSubMode(), 20, rstretch, rstrength, 0, NULL, this->peltData, applyToAll);
	...
	if (cancel == FALSE)
	{
		for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
		{
			mMeshTopoData[ldID]->RelaxByEdgeAngle(fnGetTVSubMode(), 300, 0.0f, 0.1f, FALSE, NULL, this->peltData, applyToAll);
			if (mMeshTopoData[ldID]->GetUserCancel())
			{
				ldID = mMeshTopoData.Count();
				cancel = TRUE;
...
void UnwrapMod::fnRegularMapFromEdge()
{
	...
	for (int i = 0; i < mMeshTopoData.Count(); i++)
	{
		...
		if (ld->GetGeomEdgeSel().AnyBitSet())
		{
			...
			BitArray esel = ld->GetGeomEdgeSel();
			BitArray fsel = ld->GetFaceSel();
...
void UnwrapMod::fnStraighten()
{
	...
	for (int i = 0; i < mMeshTopoData.Count(); i++)
	{
		...
		while (fSel.NumberSet() != lastCount)
		{
			...
			if (startVert != -1)
			{
				...
				for (int j = 0; j < numConnected; j++)
				{
					...
					if (ithNeighbor < 0 || ithNeighbor >= connectionData.mVertex[startVert]->mConnectedTo.Count())
						continue;
					int neighbor = connectionData.mVertex[startVert]->mConnectedTo[ithNeighbor].mVert;
					if (neighbor < 0 || neighbor >= connectionData.mVertex.Count())
						continue;
				...
				if ((nextVert < 0) || (nextVert >= numTVVerts))
				{
					DbgAssert(0);
				}
				else
				{
					...
					while (!done)
					{
						...
						if ((nextVert < 0) || (nextVert >= numTVVerts) || (connectionData.mVertex[nextVert] == NULL))
						{
							DbgAssert(0);
							return;
						}
						...
						if (borderVerts[nextVert] &&
							(nextVert != originalVert))
						{
							done = true;
							borderVerts.ClearAll(); //this type of topology is not supported
						}
						else
						{
							borderVerts.Set(nextVert, TRUE);

							prevPoint.Set(connectionData.mVertex[nextVert]->mU, connectionData.mVertex[nextVert]->mV, 0.0f);
					...
					int smallestX = 0; // INT_MAX
					int smallestY = 0;
					int largestX = 0; // INT_MIN
					int largestY = 0;
						...
						if (borderVerts[j])
						{
							...
							for (int k = 0; k < v->mVisibleEdgeCount; k++)
							{
								...
								if (v->mConnectedTo[ith].mBorderEdge)
								{
									...
									if (v->IsBorderEdge(opposing)) //(opposing >= 0)
									{
										corner = FALSE;
										break;
									}
		...
		ld->SetFaceSelectionByRef(holdSel);
	}
...
void UnwrapMod::AlignByPivot(BOOL vertical)
{
	...
	for (int i = 0; i < mMeshTopoData.Count(); i++)
	{
		...
		BitArray vsel = mMeshTopoData[i]->GetTVVertSel();
		if (vsel.AnyBitSet())
		{
			for (int j = 0; j < numVerts; j++)
			{
				if (vsel[j])
				{
					...
					p[id] = val;
					mMeshTopoData[i]->SetTVVert(t, j, p);
				}
			}
		}
		//auto pin selected verts if needed
		if (pblock->GetInt(unwrap_autopin) && GetLivePeelModeEnabled())
			fnPinSelected(GetMeshTopoDataNode(i));
	}
	...
	if (GetLivePeelModeEnabled()) //upde the live peel solve if in that mode
	{
		AutoPackDisabledGuard guard(this);
		mLSCMTool.Solve(true);
	}
...
void UnwrapMod::fnSetTypeInLinkUV(bool newVal)
{
	if(mTypeInLinkUV != newVal)
	{
		mTypeInLinkUV = newVal;

		ToolBarFrame *bar =  mUIManager.GetToolBarFrame(_T("_InternalTypeIn"));
		if(bar)
		{
			ICustToolbar *itoolBar = bar->GetToolBar();
			if(itoolBar)
			{				
...
bool UnwrapMod::fnGetTypeInLinkUV() const
{
	return mTypeInLinkUV;
}

HWND UnwrapMod::fnGetEditorHWND() const
{
	return (IsWindow(hDialogWnd)) ? hDialogWnd : NULL;
}

HWND UnwrapMod::fnGetEditorViewHWND() const
{
	return (IsWindow(hView)) ? hView : NULL;
}

BOOL UnwrapMod::fnGetTileGridVisible() const
{
	return tileGridVisible;
}

void UnwrapMod::fnSetTileGridVisible( BOOL bVisible )
{
	if (bVisible != tileGridVisible)
	{
		tileGridVisible = bVisible;
		InvalidateView();
	}
}

void UnwrapMod::fnSetDisplayPixelUnits(BOOL bDisplayPixelUnits)
{
    SetDisplayPixelUnits( (bDisplayPixelUnits!=FALSE), true );
}

BOOL UnwrapMod::fnGetDisplayPixelUnits() const
{
    return (GetDisplayPixelUnits()? TRUE:FALSE);
}

float UnwrapMod::fnGetCheckerTiling() const
{
	return fCheckerTiling;

}
void UnwrapMod::fnSetCheckerTiling(const float tiling)
{
	if(fCheckerTiling != tiling)
	{
		fCheckerTiling = tiling;
		if (fCheckerTiling < 1.0) fCheckerTiling = 1.0f;
		if (fCheckerTiling > 500.0) fCheckerTiling = 500.0f;

		UpdateCheckerTiling();
	}
}

BOOL UnwrapMod::fnGetNonSquareApplyBitmapRatio() const
{
	return mNonSquareApplyBitmapRatio;
}

void UnwrapMod::fnSetNonSquareApplyBitmapRatio(BOOL bApplyRatio)
{
	mNonSquareApplyBitmapRatio = bApplyRatio;
}
