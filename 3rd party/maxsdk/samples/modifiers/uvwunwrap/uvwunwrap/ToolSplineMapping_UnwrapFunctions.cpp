...
//	see if we have a node
	if (node)
	{
		...
		if (nos.obj->IsShapeObject()) 
		{
			...
			int numberOfCurves = pathOb->NumberOfCurves(t);
...
void	UnwrapMod::fnSplineMap()
{
	...
	for (int ldID = 0; ldID < mCT; ldID++)
	{
		...
		int splineIndex = mSplineMap.GetActiveSpline();
		{
			...
			BitArray facesToBeMapped = ld->GetFaceSel();
			if (useAllFaces)
			...
			for (int i = 0; i < ld->GetNumberFaces(); i++)
			{
				...
				{			
					...
					for (int j = 0 ; j < degree; j++)
					{
						...
						{
							geoToUVW[geoIndex] = ld->AddTVVert(t,uvw[geoIndex],i,j,FALSE,&deadVerts);
						}
						else
						{
						...
						if (ld->GetFaceCurvedMaping(i))
						{
							if (ld->GetFaceHasVectors(i))
							{
								...
								if (geoToUVW[geoIndex] == -1)
								{
									geoToUVW[geoIndex] = ld->AddTVHandle(t,uvw[geoIndex],i,j*2,FALSE,&deadVerts);
								}
								else
								{
								...
								if (geoToUVW[geoIndex] == -1)
								{
									geoToUVW[geoIndex] = ld->AddTVHandle(t,uvw[geoIndex],i,j*2+1,FALSE,&deadVerts);
								}
								else
								{
								...
								if (ld->GetFaceHasInteriors(i))
								{
									...
									if (geoToUVW[geoIndex] == -1)
									{
										geoToUVW[geoIndex] = ld->AddTVInterior(t,uvw[geoIndex],i,j,FALSE,&deadVerts);
									}
									else
									{
...
//first fix the V seam
						...
						if (inQuad1 && inQuad4)
						{
							for (int j = 0 ; j < degree; j++)
							{
								...
								if (uvw.y < 0.25f)
								{
									...
									ld->AddTVVert(t,uvw,i,j,FALSE,&deadVerts);
									...
									if (ld->GetFaceCurvedMaping(i))
									{
										if (ld->GetFaceHasVectors(i))
										{
											...
											{
												uvw.y += 1.0f;
												ld->AddTVHandle(t,uvw,i,j*2,FALSE,&deadVerts);
											}
											...
											{
												uvw.y += 1.0f;
												ld->AddTVHandle(t,uvw,i,j*2+1,FALSE,&deadVerts);
											}
			...
			if (projType != kPlanar)
			{
				...
				if (manualSeams && ld->mSeamEdges.GetSize())
				{
					...
					BitArray uvSplitEdges = ld->GetTVEdgeSel();
