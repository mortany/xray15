...
//				indexList.Append(1,&i);
		...
		//restore selections
		ld->SetTVEdgeSel(originalESel);
		ld->SetTVVertSel(originalVSel);
	}
}
...
void SketchMode::Apply(HWND hWnd, IPoint2 m)
{
	...
	if (mod->sketchType == SKETCH_LINE)
	{
		...
		MeshTopoData* pMeshTopoData = NULL;
		int index = -1;
		int numItemsOfIndexList = indexList.Count();
		if(numItemsOfIndexList > 0)
		{
			pMeshTopoData = indexList[0].mLD;
			index = indexList[0].mIndex;
		}
		pointList[0] = new Point3(mod->SnapPoint(p,pMeshTopoData,index));
		...
		if(numItemsOfIndexList > 0)
		{
			pMeshTopoData = indexList[numItemsOfIndexList-1].mLD;
			index = indexList[numItemsOfIndexList-1].mIndex;
		}
		pointList[1] =  new Point3(mod->SnapPoint(p,pMeshTopoData,index));
...
//						theHold.Put(new TVertRestore(mod,FALSE));
			...
			else if (mod->sketchSelMode == SKETCH_SELDRAG)  //we are in a drag sel mode
			{
				if (pointCount == 0) 
				{
					...
					SetCursor(mod->circleCur[MaxSDK::UIUnScaled(mod->sketchCursorSize)]);
...
void  UnwrapMod::fnSketchByNode(Tab<int> *indexList,Tab<Point3*> *positionList,INode *node)
{
	int indexCount = indexList->Count();
	Tab<SketchIndexData> newData;
	newData.SetCount(indexCount);
...
void  UnwrapMod::fnSketchNoParams()
{
	...
	if (sketchSelMode == SKETCH_SELCURRENT)  //we are in a drag sel mode
	{
		...
		for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++) //if not in vertex mode convert to vertex
		{
	  ...
	  case WM_INITDIALOG:
		  {
			  ...
			  iCursorSize = SetupIntSpinner(
				  ...
				  1,15, MaxSDK::UIUnScaled(mod->sketchCursorSize));

			  mod->SetSketchDialogPos();
			  break;
		  }
	  ...
	  case IDC_OK:
		  {
			  ...
			  mod->sketchCursorSize = MaxSDK::UIScaled(iCursorSize->GetIVal());
	  ...
	  case IDC_DEFAULT:
		  {
			  ...
			  mod->fnSetAsDefaults(SKETCHSECTION);
			  break;
		  }
