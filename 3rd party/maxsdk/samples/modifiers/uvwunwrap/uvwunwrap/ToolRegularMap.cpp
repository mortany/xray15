...
RegularMap::RegularMap() : mMod(NULL)
{
...
bool RegularMap::Init(UnwrapMod *mod, MeshTopoData *md)
{
	...
	if (mMd->GetFaceSel().NumberSet()  && (mod->fnGetTVSubMode() == TVFACEMODE))
		useSelection = true;
	...
	if (md->GetMesh())
	{
		...
		AdjFaceList* af = md->GetMeshAdjFaceList();
...
//		else if (uvws[i].mIthPolyEdge == (ithTargetEdge+1)%uvws.Count())
		...
		else
		{
			...
			uvIndex = mMd->AddTVVert(t,uvws[i].mUVW);
		}
...
bool RegularMap::StartNewCluster(int nfaceIndex)
{
	...
	for (int i = 0; i < uvws.Count(); i++)
	{
		...
		int uvIndex = mMd->AddTVVert(t,uvws[i].mUVW);
		geomToTVLookup[geomIndex] = uvIndex; 
	}
...
void RegularMap::Expand(RegularMapExpansions expandBy) 
{
	...
	if (expandBy == regularmap_expand_loop) 
	{
		...
		BitArray esel = mMd->GetGeomEdgeSel();
		 ...
		 (expandBy == regularmap_expand_planar) )
	{
		...
		BitArray edgeSkip = mMd->GetGeomEdgeSel();
		edgeSkip.ClearAll();
		 ...
		 (expandBy == regularmap_expand_planar) )
	{
		...
		BitArray edgeSel = mMd->GetGeomEdgeSel();
		edgeSel.ClearAll();
	...
	for (int i = 0; i < 2; i++)  //we do the expansion in 2 passes first the U direction and then the v
	{
		while (mProcessedFaces.NumberSet() != lastCount)
		{
			...
			mMd->SetGeomEdgeSel(holdEdgeSel);
		}	
	}
...
//FIX
				...
				for (int i = 0; i < mPolygon.Count(); i++)
				{
					...
					if (mPolygon[i]->mCusterID == (int)mCurrentClusterID)
					{
						bool addFaces = false;
				...
				mMd->WeldFaces(weldFaces,mAutoWeld,mAutoWeldLimit);
/*
				if (mAutoWeld)
...
*/
	...
	mMod->SyncTVFromGeomSelection(mMd);
...
#ifdef DEBUG_TV_VERT_CONNECTED_TO_EDGES
		...
		if (startEdge != -1)
		{
			...
			if (startEdge != -1)
			{
				...
				for (int i = 0; i < edgeList.Count(); i++)
				{
					...
					pa[axis] = val;
					mMd->SetTVVert(t,tva,pa);

					Point3 pb = mMd->GetTVVert(tvb);
					pb[axis] = val;
					mMd->SetTVVert(t,tvb,pb);
					tvLen += Length(pb-pa);
				...
				{					
					...
					mMd->SetTVVert(t,tvVerts[i+1],newPoint);
				}
...
void RegularMap::FitView(int level)
{
	if (level == 1)
	{
		...
		if (newFaces.NumberSet() > 0)
		{
			...
			BitArray holdFaces = mMd->GetFaceSel();
			BitArray holdEdges = mMd->GetGeomEdgeSel();
			BitArray holdTVEdges = mMd->GetTVEdgeSel();
			...
			mMd->SetGeomEdgeSel(holdEdges);
			mMd->SetTVEdgeSel(holdTVEdges);
		}
