...
void ToolBarFrame::Init(HINSTANCE hInstance) 
{
	mPopup = 0;
...
void ToolBarFrame::AddButton(int id, ToolButtonItem item, const MCHAR* toolTip)
{
...
void ToolBarFrame::AddButton(int id, ToolButtonItem item, FlyOffIconList& iconList, int flyOffValue)
{
	mToolBar->AddTool(item);
	ICustButton *but = mToolBar->GetICustButton(id);
	if (but)
	{
		but->SetFlyOffIconsByName(iconList, GetCOREInterface()->GetFlyOffTime(), flyOffValue, FLY_VARIABLE);
		ReleaseICustButton(but);
	}
}
