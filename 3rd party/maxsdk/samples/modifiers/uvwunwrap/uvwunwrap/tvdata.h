...
//index into the geometric vertlist
	...
	UVW_TVFaceClass() : MatID(0), FaceIndex(0)
		{
		count = 3;
		flags = 0;
		vecs = NULL;
	...
	//we track whether the matid is set since we only want to copy matid that have been changed
	//these let you control the mat id set property
	BOOL IsMatIDSet();
	void MatIDSet();
	void ClearMatIDSet();
private:
...
//indices into the vert list
	...
	UVW_TVEdgeDataClass() : a(0), avec(0), b(0), bvec(0), flags(0), lookupIndex(0)
		{
			ga = -1;
			gb = -1;
		}
...
public:
	...
	bool EdgeListFromPoints(Tab<int> &outputEdges, int a, int b, const BitArray &candidateEdges);

	int GetNextEdge(int currentEdgeIndex, int cornerVert, int currentFace);
	void SplitUVEdges(BitArray esel, bool& bValidTopoForBreakEdges);
	...
	TopoChangedManager& GetTopoChangedManager();
	void SetTopoChangedManager(const TopoChangedManager& tChangedManager);
...
private:
	TopoChangedManager mTopoChangedManager;

	void BuildPointSelFromEdgeSel(const BitArray &edgeSel, BitArray &pointSel);
	void BuildNewEdgesAtVert(const BitArray &pointSel, Tab<VEdges*> &edgesAtVert);

	void SplitUVEdgeAtVert(const Tab<VEdges*> &edgesAtVertex, const int vIndex, const BitArray &edgeSel, bool& bValidTopoForBreakEdges);
	void SplitUVEdgeAtEdge(const int eIndex, BitArray &processedFaces, bool &isFirstFacetAtVert, const int vIndex, const Tab<VEdges*> &edgesAtVertex, const BitArray &edgeSel, bool& bValidTopoForBreakEdges);
	void SplitUVEdgeAtFacet(const int eIndex, const int fIndex, int vIndexForFacet, const int vIndex, BitArray &processedFaces, const Tab<VEdges*> & edgesAtVertex, const BitArray & edgeSel, bool& bValidTopoForBreakEdges);

	int GetVertIndexAtFacet(bool &isFirstFacetAtVert, const int vIndex);
	void ReplaceVertAtFace(const int vIndexForFacet, const int vIndex, const int curFIndex);
	int GetNextEdgeIndex(const Tab<VEdges*> & edgesAtVertex, const int vIndex, const int curEIndex, int curFIndex);
	int GetNextFaceIndex(const int curEdgeIndex, const int curFaceIndex);

	void BuildVConnections(Tab<VConnections*> &connects, const BitArray &candidateEdges);
	void FreeVConnections(Tab<VConnections*> &connects);
	static bool IsTopoConnected(const Tab<VConnections*> &connects, int source, int target);
};
...
class GeomTVEdgeIndices
{
public:
	int tvIndex1;
	int tvIndex2;

	GeomTVEdgeIndices() :tvIndex1(-1), tvIndex2(-1) {}
	GeomTVEdgeIndices(int tv1, int tv2) :tvIndex1(tv1), tvIndex2(tv2) {}
};

class GeoTVEdgesMap
{
public:
	void addGeomTVEdgePair(int geomIndex, int tvIndex);

	std::map<int, GeomTVEdgeIndices> gtvInfo; // data is arranged following <gEdgeIndex, <tEdgeIndex1, tEdgeIndex2>>
};

class ReplaceVertMap
{
public:
	int findLiveVert(int otv);

	void addReplacePair(int otv, int ntv);

	std::map<int, int> tvMap; // <oldTVVert, newTVVert>
};
