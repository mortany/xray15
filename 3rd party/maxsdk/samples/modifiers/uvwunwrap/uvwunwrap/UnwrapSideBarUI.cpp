...
SideBarUI::SideBarUI() : mRollupHwnd(NULL)
{
	mMod = NULL;
}
...
void SideBarUI::SetDefaults(int index, HWND parentWindow)
{
	if (index == 100) //quick transform
	{
		...
		mMod->AddActionToToolbar(bar1, ID_ALIGNH_BUTTONS);
	...
	else if (index == 101)
	{
		...
		mMod->AddActionToToolbar(bar2, ID_ALIGNV_BUTTONS);
	...
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S5_3_TOOLBAR), index, _M("__InternalPeel3"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalPeel3"));

		mMod->AddActionToToolbar(bar2, ID_AUTO_PACK);
	}
	else if (index == 150)
	{
	...
	else if (index == 180) // brush rollout , paint move mode 
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S8_1_TOOLBAR), index, _M("__InternalBrushCustBar0"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar0"));
		mMod->AddActionToToolbar(bar2, ID_PAINT_MOVE_BRUSH);
	}
	else if (index == 181) // brush rollout , strength spinner editor.
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S8_2_TOOLBAR), index, _M("__InternalBrushCustBar1"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar1"));
		mMod->AddActionToToolbar(bar2, ID_BRUSH_STRENGTHEDIT);
	}
	else if (index == 182) // brush rollout, falloff spinner editor
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S8_3_TOOLBAR), index, _M("__InternalBrushCustBar2"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar2"));
		mMod->AddActionToToolbar(bar2, ID_BRUSH_FALLOFFEDIT);
	}
	else if (index == 183) // brush rollout, falloff type
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S8_4_TOOLBAR), index, _M("__InternalBrushCustBar3"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar3"));
		mMod->AddActionToToolbar(bar2, ID_BRUSH_FALLOFF_TYPE);
	}
	else if (index == 184) // brush rollout, relax move mode
	{
		mMod->GetUIManager()->AppendToolBar(GetDlgItem(parentWindow, IDC_S8_5_TOOLBAR), index, _M("__InternalBrushCustBar4"));

		ToolBarFrame* bar2 = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar4"));
		mMod->AddActionToToolbar(bar2, ID_RELAX_MOVE_BRUSH);
	}
}
...
void SideBarUI::LoadInActions(int index)
{
	...
	if (index == 100)
		...
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalPeel3"));
	else if (index == 150)
		...
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar0"));
	else if (index == 181)
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar1"));
	else if (index == 182)
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar2"));
	else if (index == 183)
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar3"));
	else if (index == 184)
		bar = mMod->GetUIManager()->GetToolBarFrame(_M("__InternalBrushCustBar4"));
...
void SideBarUI::Setup(HINSTANCE hInstance, HWND rollupHWND, const MCHAR *iniFile)
{
	...
	irollup->AppendRollup(hInstance, MAKEINTRESOURCE(IDD_SIDEBAR_ROLLUP8), SideBar1RollupDialogProc, GetString(IDS_BRUSH), (LPARAM)mMod);
	...
	// put the brush rollout right after transform rollout
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 180, irollup->GetPanelDlg(1), GetDlgItem(irollup->GetPanelDlg(1), IDC_S8_1_TOOLBAR)) > 0)
		LoadInActions(180);
	else
		SetDefaults(180, irollup->GetPanelDlg(1));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 181, irollup->GetPanelDlg(1), GetDlgItem(irollup->GetPanelDlg(1), IDC_S8_2_TOOLBAR)) > 0)
		LoadInActions(181);
	else
		SetDefaults(181, irollup->GetPanelDlg(1));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 182, irollup->GetPanelDlg(1), GetDlgItem(irollup->GetPanelDlg(1), IDC_S8_3_TOOLBAR)) > 0)
		LoadInActions(182);
	else
		SetDefaults(182, irollup->GetPanelDlg(1));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 183, irollup->GetPanelDlg(1), GetDlgItem(irollup->GetPanelDlg(1), IDC_S8_4_TOOLBAR)) > 0)
		LoadInActions(183);
	else
		SetDefaults(183, irollup->GetPanelDlg(1));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 184, irollup->GetPanelDlg(1), GetDlgItem(irollup->GetPanelDlg(1), IDC_S8_5_TOOLBAR)) > 0)
		LoadInActions(184);
	else
		SetDefaults(184, irollup->GetPanelDlg(1));

	// reshape
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 110, irollup->GetPanelDlg(2), GetDlgItem(irollup->GetPanelDlg(2), IDC_S2_1_TOOLBAR)) > 0)
		LoadInActions(110);
	else
		SetDefaults(110, irollup->GetPanelDlg(2));

	// stitch
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 120, irollup->GetPanelDlg(3), GetDlgItem(irollup->GetPanelDlg(3), IDC_S3_1_TOOLBAR)) > 0)
		LoadInActions(120);
	else
		SetDefaults(120, irollup->GetPanelDlg(3));

	// explode
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 130, irollup->GetPanelDlg(4), GetDlgItem(irollup->GetPanelDlg(4), IDC_S4_1_TOOLBAR)) > 0)
		LoadInActions(130);
	else
		SetDefaults(130, irollup->GetPanelDlg(4));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 131, irollup->GetPanelDlg(4), GetDlgItem(irollup->GetPanelDlg(4), IDC_S4_2_TOOLBAR)) > 0)
		LoadInActions(131);
	else
		SetDefaults(131, irollup->GetPanelDlg(4));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 132, irollup->GetPanelDlg(4), GetDlgItem(irollup->GetPanelDlg(4), IDC_S4_3_TOOLBAR)) > 0)
		LoadInActions(132);
	else
		SetDefaults(132, irollup->GetPanelDlg(4));

	// peel
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 140, irollup->GetPanelDlg(5), GetDlgItem(irollup->GetPanelDlg(5), IDC_S5_1_TOOLBAR)) > 0)
		LoadInActions(140);
	else
		SetDefaults(140, irollup->GetPanelDlg(5));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 141, irollup->GetPanelDlg(5), GetDlgItem(irollup->GetPanelDlg(5), IDC_S5_2_TOOLBAR)) > 0)
		LoadInActions(141);
	else
		SetDefaults(141, irollup->GetPanelDlg(5));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 142, irollup->GetPanelDlg(5), GetDlgItem(irollup->GetPanelDlg(5), IDC_S5_0_TOOLBAR)) > 0)
		LoadInActions(142);
	else
		SetDefaults(142, irollup->GetPanelDlg(5));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 143, irollup->GetPanelDlg(5), GetDlgItem(irollup->GetPanelDlg(5), IDC_S5_3_TOOLBAR)) > 0)
		LoadInActions(143);
	else
		SetDefaults(143, irollup->GetPanelDlg(5));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 144, irollup->GetPanelDlg(5), GetDlgItem(irollup->GetPanelDlg(5), IDC_S5_4_TOOLBAR)) > 0)
		LoadInActions(144);
	else
		SetDefaults(144, irollup->GetPanelDlg(5));

	// arrange element
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 150, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_1_TOOLBAR)) > 0)
		LoadInActions(150);
	else
		SetDefaults(150, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 151, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_2_TOOLBAR)) > 0)
		LoadInActions(151);
	else
		SetDefaults(151, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 152, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_3_TOOLBAR)) > 0)
		LoadInActions(152);
	else
		SetDefaults(152, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 153, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_4_TOOLBAR)) > 0)
		LoadInActions(153);
	else
		SetDefaults(153, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 154, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_5_TOOLBAR)) > 0)
		LoadInActions(154);
	else
		SetDefaults(154, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 155, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_6_TOOLBAR)) > 0)
		LoadInActions(155);
	else
		SetDefaults(155, irollup->GetPanelDlg(6));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 156, irollup->GetPanelDlg(6), GetDlgItem(irollup->GetPanelDlg(6), IDC_S6_7_TOOLBAR)) > 0)
		LoadInActions(156);
	else
		SetDefaults(156, irollup->GetPanelDlg(6));

	// element properties
	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 160, irollup->GetPanelDlg(7), GetDlgItem(irollup->GetPanelDlg(7), IDC_S7_1_TOOLBAR)) > 0)
		LoadInActions(160);
	else
		SetDefaults(160, irollup->GetPanelDlg(7));

	if (bResult && pUIManager->LoadSingleFromIniFile(iniFile, 161, irollup->GetPanelDlg(7), GetDlgItem(irollup->GetPanelDlg(7), IDC_S7_2_TOOLBAR)) > 0)
		LoadInActions(161);
	else
		SetDefaults(161, irollup->GetPanelDlg(7));
...
void SideBarUI::TearDown()
{
	...
	if (mMod->GetUIManager()->IsChecked(ID_PEEL_DETACH))
		mMod->sPeelDetach = true;
	else
		mMod->sPeelDetach = false;

	if (mMod->GetUIManager()->IsChecked(ID_AUTO_PACK))
		mMod->sAutoPack = true;
	else
		mMod->sAutoPack = false;
