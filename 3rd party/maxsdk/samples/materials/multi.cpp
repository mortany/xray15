...
// void     SetNSubMtls( int num );
   ...
   void     SetSubMtlAndName( int mtlid, Mtl *m, TSTR &subMtlName ) override;
   ...
   void     GetClassName( TSTR &s ) override    { s = GetString(IDS_RB_MULTISUBOBJECT); }  
   ...
   Mtl *       GetSubMtl( int i ) override 
            { 
               ...
               int idx = hashTab.GetIndex( i );
               return ( idx == -1 ) ? nullptr : subMtl[ hashTab[idx] ]; // 4/13/01 4:44pm --MQM-- from subMtl[idx] to subMtl[ hashTab[idx] ]
#else
               int j = FindSubMtl( i );
               return ( j >= 0 ) ? subMtl[j] : nullptr;
#endif
            }
...
class MultiClassDesc:public ClassDesc2 {
   ...
   const TCHAR *  ClassName() override { return GetString(IDS_RB_MULTISUBOBJECT_CDESC); } // mjm - 2.3.99
   SClass_ID      SuperClassID() override { return MATERIAL_CLASS_ID; }
   Class_ID       ClassID() override { return MULTI_MATERIAL_CLASS_ID; }
...
void MultiDlg::SetTime(TimeValue t) {
   if (t!=curTime) {
      ...
      //InvalidateRect(hPanelBasic,nullptr,0);
      }
   }
...
void Multi::AddMtl(ReferenceTarget *rt, int id, const TCHAR *name) {
   ...
   subMtl[n-1] = nullptr;  // orb 08-24-2001 Fixing Defects 307753 & 306999
