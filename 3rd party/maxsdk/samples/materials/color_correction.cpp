	...
	if (m_Map) 
	{
		...
		for( int y = 0; y < h; ++y ) {
			for( int x = 0; x < w; ++x, data += 4 ) {
				...
				data[2] = (unsigned char)( clamp(res.r * 0xFF, 0, 0xFF ));
				data[1] = (unsigned char)( clamp(res.g * 0xFF, 0, 0xFF ));
				data[0] = (unsigned char)( clamp(res.b * 0xFF, 0, 0xFF ));
				data[3] = (unsigned char)( clamp(res.a * 0xFF, 0, 0xFF ));
			}
		}
	} 
	else 
...
void ColorCorrection::Texture::Update( TimeValue t, Interval& valid )
{
	...
	m_AdvanceG[AdvanceIndex::lift] = m_ParamBlock->GetFloat( BlockParam::RGBLift,  t );
