...
public:
		...
		// overiding one map channel with the camera projection
	...
	public:
		...
		virtual bool OnPreParameterTranslate() { return true;}
...
static ParamBlockDesc2 cameramaptexture_param_blk ( cameramaptexture_params, _T("params"),  0, &CameraMapTextureDesc, 
	...
	pb_backface,	_T("backFace"), TYPE_BOOL, 0, IDS_BACKFACE,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_BACKFACECHECK,
		p_end,

	pb_usezbuffer,	_T("useZBuffer"), TYPE_BOOL, 0, IDS_USEZBUFFER,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_ZBUFFERCHECK,
		p_end,
	...
	pb_usemask,	_T("useMask"), TYPE_BOOL, 0, IDS_USEMASK,
		p_default,		FALSE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_MASKCHECK,
		p_end,

	pb_maskuseprojection,	_T("maskUsesProjection"), TYPE_BOOL, 0, IDS_MASKUSESPROJECTION,
		p_default,		TRUE,
		p_ui,			TYPE_SINGLECHECKBOX, IDC_MASKUSEPROJECTIONCHECK,
		p_end,
	...
	ShaderParameterType)
{
	...
	if (_tcscmp(shaderParamName,_M("camera_matViewproj")) == 0 && 
		NULL != cameraNode && NULL != tex)
	{
		CameraState cs;
		Interval iv;

		CameraObject* pCameraObject = NULL;
		const ObjectState& os = cameraNode->EvalWorldState(t);
		pCameraObject = (CameraObject *)os.obj;
		pCameraObject->EvalCameraState(t,iv,&cs);

		// Use render setting for target width & height for default
		Interface* pInterface = GetCOREInterface();
		float targetWidth = (float)pInterface->GetRendWidth();
		float targetHeight = (float)pInterface->GetRendHeight();

		BitmapTex* pBitmapTex = GetIBitmapTextInterface(tex);
		if (NULL != pBitmapTex)
		{
			Bitmap* pBitmap = pBitmapTex->GetBitmap(t);
			if (pBitmap != NULL)
			{
				targetWidth = pBitmap->Width();
				targetHeight = pBitmap->Height();
			}
		}
...
#define MATRIX_EPSILON   0.000001f
		...
		float deviceAspect = pInterface->GetRendApect();
		Matrix44 projectionMatrix;
		projectionMatrix.MakeIdentity();
		if (!cs.isOrtho)
		{
			projectionMatrix._11 = 1.0f / tan(0.5f * cs.fov);
			projectionMatrix._22 = projectionMatrix._11 * ((float)targetWidth / (float)targetHeight) * deviceAspect;
		...
		else
		{
			...
			float h = w * targetHeight / targetWidth / deviceAspect; 
